...
LABEL = "skillTimelineCheckTargetContains"
CONTRACT_PATH = CONTRACTS_DIR / "skill_timeline_check_target_contains_native.json"
TAG = 0x007E
READ_KINDS = (
...
def _contract() -> dict[str, Any]:
    contract, _digest = read_reviewed_contract(
        CONTRACT_PATH,
        schema="endfield.skill-timeline-check-target-contains-native-contract.v1",
        label=LABEL,
    ...
    if (
        ...
        != ["Beyond.Gameplay.Core.TargetSettings", "Beyond.Gameplay.Core.TargetSettings"]
    ):
...
def validate_current_native_contract() -> dict[str, Any]:
    """Reprove the selected route, six source calls and two TargetSettings profiles."""
    contract = _contract()
...
def decode_check_target_contains_action(reader: Reader, depth: int, tag: int, width: int) -> None:
    """Consume one reached CheckTargetContains with finite TargetSettings profiles."""
    del depth
    _contract()
    ...
    if reader.peek() == 0xFF:
        ...
        return
    reader.header(6)
    reader.take(1, "CheckTargetContains.isEnable.byte")
    for field in ("priorityLevel", "priorityOffset", "serverActionIndex"):
        reader.take(4, f"CheckTargetContains.{field}.raw4")
