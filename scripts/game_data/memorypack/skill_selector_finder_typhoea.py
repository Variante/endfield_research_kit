"""Selected-build finite SelectorFinder 0x0017 SkillData nested profile.
...
LABEL = "skillSelectorFinderTyphoea"
CONTRACT_PATH = CONTRACTS_DIR / "skill_selector_finder_typhoea_native.json"
TAG = 0x0017
TYPE_NAME = "Beyond.Gameplay.Core.Selector+TyphoeaArcherySelectedFinder+Data"
WRAPPER_NAME = "Beyond.MemoryPack.Beyond_Gameplay_Core_Selector_TyphoeaArcherySelectedFinder_DataForMemoryPack"
...
def _catalog() -> dict[str, Any]:
    ...
    route = catalog.get("families", {}).get("SelectorFinder", [])[TAG]
    switch = catalog.get("switches", {}).get("SelectorFinder", {})
    if (
        ...
        or switch.get("entryCount") != len(catalog["families"]["SelectorFinder"])
...
def validate_current_native_contract() -> dict[str, Any]:
    ...
    if (
        int(catalog["switches"]["SelectorFinder"]["tableVa"], 16)
...
def decode_typhoea_selected_finder(reader: Reader) -> None:
    """Consume a reached tag-23 nested finder, retaining FF as a distinct state."""
    _contract()
    _catalog()
    if reader.peek() != TAG:
