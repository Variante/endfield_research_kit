"""Selected-build SkillData CameraRotateAction 0x0025 framing.

The reviewed native reader fixes ten source reads and six generic contexts.
Curve and BlackboardDouble children reuse separately bounded profiles; the
enclosing timeline and SkillData continuation remain the caller's evidence.
"""
...
LABEL = "skillTimelineCameraRotate"
CONTRACT_PATH = CONTRACTS_DIR / "skill_timeline_camera_rotate_native.json"
TAG = 0x0025
READ_KINDS = (
    "bool-byte", "enum32", "scalar32", "scalar32", "enum32",
    "float32-bits", "curve-profile", "scalar-payload", "enum32",
    "scalar-payload",
)
FIELD_NAMES = (
    "isEnable", "priorityLevel", "priorityOffset", "serverActionIndex",
    "blendStyle", "ccsPriority", "customCurve", "duration",
    "inputConflictStrategy", "totalAngle",
)
GENERIC_TYPES = (
    (1, "Beyond.Gameplay.Core.AbilityAction+AbilityActionData+Priority"),
    (4, "Cinemachine.CinemachineBlendDefinition+Style"),
    (6, "UnityEngine.AnimationCurve"),
    (7, "Beyond.Blackboard+BlackboardDouble"),
    (8, "Beyond.Gameplay.View.CameraControlOneTimeRotateConfig+InputConflictStrategy"),
    (9, "Beyond.Blackboard+BlackboardDouble"),
)
...
def _contract() -> dict[str, Any]:
    contract, _digest = read_reviewed_contract(
        CONTRACT_PATH,
        schema="endfield.skill-timeline-camera-rotate-native-contract.v1",
        label=LABEL,
...
def validate_current_native_contract() -> dict[str, Any]:
    """Recheck the selected route, source reads and generic type arguments."""
    contract = _contract()
    ...
    return {
        ...
        "sourceReadCount": len(READ_KINDS), "genericContextCount": len(GENERIC_TYPES),
    }


def decode_camera_rotate_action(reader: Reader, depth: int, tag: int, width: int) -> None:
    """Consume one reached tag-0x0025 action and stop at any child mismatch."""
    del depth
    _contract()
    ...
    reader.take(1, "CameraRotateAction.bool-byte")
    for _ in range(5):
        reader.take(4, "CameraRotateAction.scalar32")
    reader.curve_profile()
    reader.scalar_payload()
    reader.take(4, "CameraRotateAction.enum32")
    reader.scalar_payload()
