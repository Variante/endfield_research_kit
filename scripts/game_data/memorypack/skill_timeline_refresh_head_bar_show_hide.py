"""Selected-build SkillData RefreshHeadBarShowHideAction 0x012C framing.

The wrapper adds no members beyond the inherited action prefix. Its name is
not evidence of an observed runtime head-bar visibility change.
"""
...
LABEL = "skillTimelineRefreshHeadBarShowHide"
CONTRACT_PATH = CONTRACTS_DIR / "skill_timeline_refresh_head_bar_show_hide_native.json"
TAG = 0x012C
WRAPPER_NAME = "Beyond.MemoryPack.Beyond_Gameplay_Core_RefreshHeadBarShowHideAction_DataForMemoryPack"
TYPE_NAME = "Beyond.Gameplay.Core.RefreshHeadBarShowHideAction+Data"
...
def _contract() -> dict[str, Any]:
    contract, _digest = read_reviewed_contract(
        CONTRACT_PATH,
        schema="endfield.skill-timeline-refresh-head-bar-show-hide-native-contract.v1",
        label=LABEL,
...
def validate_current_native_contract() -> dict[str, Any]:
    """Authenticate the selected route, inheritance, four calls and Priority."""
    contract = _contract()
...
def decode_refresh_head_bar_show_hide_action(
    reader: Reader, depth: int, tag: int, width: int,
) -> None:
    """Consume one finite action, retaining all stored DWORDs as raw spans."""
    del depth
    _contract()
    ...
    reader.take(1, "RefreshHeadBarShowHide.isEnable.byte")
    for field in FIELD_NAMES[1:]:
        reader.take(4, f"RefreshHeadBarShowHide.{field}.raw4")
