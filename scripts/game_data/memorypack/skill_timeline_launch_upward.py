"""Selected-build SkillData LaunchUpwardAction 0x00DF reader.
...
LABEL = "skillTimelineLaunchUpward"
CONTRACT_PATH = CONTRACTS_DIR / "skill_timeline_launch_upward_native.json"
TAG = 0x00DF
FIELD_NAMES = (
    "isEnable", "priorityLevel", "priorityOffset", "serverActionIndex",
    "airborneEffect", "deadOption", "faceDirection", "floatingDuration",
    "floatingHeight", "immobilizedTime", "returnTrueWhen", "source",
    "speedFactorMultiplier", "target", "teammateBigStagger",
)
READ_KINDS = (
    "bool-byte", "scalar32", "scalar32", "scalar32",
    "effect-configuration-profile", "scalar32", "direction-profile",
    "scalar-payload", "scalar-payload", "raw-float32-bits", "scalar32",
    "target-profile", "raw-float32-bits", "target-profile", "bool-byte",
)
NESTED_TYPES = (
    "Beyond.Gameplay.Core.AbilityAction+AbilityActionData+Priority",
    "Beyond.Gameplay.EffectActionCfg",
    "Beyond.Gameplay.ControlledStateDeadOption",
    "Beyond.Gameplay.Core.DirectionSettings",
    "Beyond.Blackboard+BlackboardDouble",
    "Beyond.Blackboard+BlackboardDouble",
    "Beyond.Gameplay.Core.AbilityAction+ReturnTrueMethod",
    "Beyond.Gameplay.Core.TargetSettings",
    "Beyond.Gameplay.Core.TargetSettings",
)
NESTED_MEMBERS = (1, 4, 5, 6, 7, 8, 10, 11, 13)
...
def _contract() -> dict[str, Any]:
    contract, _digest = read_reviewed_contract(
        CONTRACT_PATH,
        schema="endfield.skill-timeline-launch-upward-native-contract.v1",
        label=LABEL,
...
def validate_current_native_contract() -> dict[str, Any]:
    ...
    for name in ("buff_9a_native.json", "buff_ec_native.json", "buff_b2_native.json"):
...
def decode_launch_upward_action(reader: Reader, depth: int, tag: int, width: int) -> None:
    """Consume one finite fifteen-member action with distinct child spans."""
    del depth
    _contract()
    ...
    reader.take(1, "LaunchUpwardAction.bool-byte")
    for _ in range(3):
        reader.take(4, "LaunchUpwardAction.scalar32")
    reader.effect_configuration_profile()
    reader.take(4, "LaunchUpwardAction.scalar32")
    reader.direction_profile()
    reader.scalar_payload()
    reader.scalar_payload()
    reader.take(4, "LaunchUpwardAction.float32-bits")
    reader.take(4, "LaunchUpwardAction.scalar32")
