"""Selected-build SkillData DoOnceAction 0x00A0 reader.

The reviewed native wrapper reads four inherited primitives followed by one
SequenceActionData object. Child action routes retain the enclosing reader's
finite grammar and exact stop behavior.
"""
...
LABEL = "skillTimelineDoOnce"
CONTRACT_PATH = CONTRACTS_DIR / "skill_timeline_do_once_native.json"
TAG = 0x00A0
FIELD_NAMES = (
...
def _contract() -> dict[str, Any]:
    contract, _digest = read_reviewed_contract(
        CONTRACT_PATH,
        schema="endfield.skill-timeline-do-once-native-contract.v1",
        label=LABEL,
...
def validate_current_native_contract() -> dict[str, Any]:
    """Authenticate the selected union route, source order, and child type."""
    contract = _contract()
...
def decode_do_once_action(reader: Reader, depth: int, tag: int, width: int) -> None:
    """Consume one action and recurse only through the reviewed child grammar."""
    _contract()
    ...
    reader.take(1, "DoOnceAction.bool-byte")
    for _ in range(3):
        reader.take(4, "DoOnceAction.scalar32")
