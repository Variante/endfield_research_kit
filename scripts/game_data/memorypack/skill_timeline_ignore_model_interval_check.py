"""Selected-build SkillData IgnoreModelIntervalCheck 0x00D0 reader.

The four source reads are inherited AbilityAction members. Runtime interval
semantics are outside this stored-format contract.
"""
...
LABEL = "skillTimelineIgnoreModelIntervalCheck"
CONTRACT_PATH = CONTRACTS_DIR / "skill_timeline_ignore_model_interval_check_native.json"
TAG = 0x00D0
FIELD_NAMES = ("isEnable", "priorityLevel", "priorityOffset", "serverActionIndex")
READ_KINDS = ("bool-byte", "scalar32", "scalar32", "scalar32")
...
def _contract() -> dict[str, Any]:
    contract, _digest = read_reviewed_contract(
        CONTRACT_PATH,
        schema="endfield.skill-timeline-ignore-model-interval-check-native-contract.v1",
        label=LABEL,
...
def validate_current_native_contract() -> dict[str, Any]:
    """Authenticate selected dispatch, wrapper, and four ordered source calls."""
    contract = _contract()
...
def decode_ignore_model_interval_check_action(
    reader: Reader, depth: int, tag: int, width: int
) -> None:
    """Consume one finite four-member action, including a possible null wrapper."""
    del depth
    _contract()
    ...
    reader.take(1, "IgnoreModelIntervalCheck.bool-byte")
    for _ in range(3):
        reader.take(4, "IgnoreModelIntervalCheck.scalar32")
