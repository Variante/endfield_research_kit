"""Selected-build SkillData BlowOffEnemyAction 0x001D storage framing.

The reviewed Buff frontier-nine contract authenticates the fourteen ordered
source reads and nested types. The byte reader keeps stored extents separate
from runtime knockback, movement, and death-state behavior.
"""
...
def validate_current_native_contract() -> dict[str, Any]:
    """Reprove the existing source route on the installed native binaries."""
    route = _route()
...
def decode_blow_off_enemy_action(
    reader: Reader, depth: int, tag: int, width: int,
) -> None:
    """Consume one finite fourteen-member wrapper and its nested profiles."""
    del depth
    _route()
    ...
    reader.take(1, "BlowOffEnemyAction.anonymous-byte")
    for _ in range(3):
        reader.take(4, "BlowOffEnemyAction.anonymous-scalar32")
    reader.target_profile()
    reader.scalar_payload()
    reader.scalar_payload()
    reader.take(4, "BlowOffEnemyAction.anonymous-enum32")
    reader.direction_profile()
    reader.scalar_payload()
    reader.take(1, "BlowOffEnemyAction.anonymous-bool-byte")
    reader.take(1, "BlowOffEnemyAction.anonymous-bool-byte")
    reader.target_profile()
    reader.scalar_payload()
