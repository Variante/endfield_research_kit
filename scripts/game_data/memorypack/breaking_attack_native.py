"""Validate the selected native BreakingAttackCalculation.Evaluate proof.
...
CONTRACT = CONTRACTS_DIR / "breaking_attack_calculation_native.json"
SCHEMA = "endfield.breaking-attack-calculation-native-contract.v1"
REPORT_SCHEMA = "endfield.breaking-attack-calculation-native-audit.v1"
DEFAULT_OUTPUT = REPO_ROOT / "reports/game_data/breaking_attack_calculation_native.json"
...
def _validate_body(image: NativeImage, contract: dict[str, Any]) -> dict[str, Any]:
    ...
    if set(methods) != {
        "evaluate", "getAttribute", "blackboardValue", "patchCheck", "getPatch", "patchWrapper"
    }:
    ...
    for key, row in methods.items():
        ...
        image.validate_method_row(selected, label="breaking-attack")
    ...
    if set(callee_signatures) != {"getAttribute", "blackboardValue", "patchWrapper"}:
    ...
    if len(expected_fields) != 2 or {row["name"] for row in expected_fields} != {
        "multiplier", "atkScale"
    }:
    ...
    enum_spec = contract["attributeEnumMembers"]
    ...
    selected_enums = []
    for expected in enum_spec["members"]:
        found = [row for row in members if row["name"] == expected["name"]]
        if len(found) != 1 or found[0]["id"] != expected["id"] or found[0]["token"] != expected["token"]:
            raise ValueError(f"native-attribute-enum={enum_spec['type']}.{expected['name']}")
        selected_enums.append(found[0])
    if len(selected_enums) != 2 or {row["name"] for row in selected_enums} != {
        "Atk", "BreakingAttackDamageTakenScalar"
    }:
        raise ValueError("contract-attribute-enum-set")
    ...
    for call in calls:
        ...
        target = _relative_target(image, call_site, b"\xe8", 4)
    ...
    for branch in branches:
        ...
        target = _relative_target(
            image, branch_site, bytes.fromhex(branch["opcode"]), branch["displacementBytes"],
        )
    ...
    return {
        ...
        "fields": field_rows,
        "attributeEnumMembers": selected_enums,
...
def audit(*, gameassembly: Path, metadata: Path, contract_path: Path = CONTRACT) -> dict[str, Any]:
    contract, digest = read_reviewed_contract(
        contract_path, schema=SCHEMA, label="breaking-attack", status="validated",
    )
    ...
    if gate.status != NATIVE_EVIDENCE_VALIDATED:
        return report
    image = NativeImage(gameassembly, metadata, label="breaking-attack")
    try:
