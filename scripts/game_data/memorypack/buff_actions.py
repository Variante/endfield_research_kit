...
class Reader:
    ...
    def _action(self,depth,tag,width):
        ...
        if tag not in (201,118,236,80,287,180,86,146,87,91,60,120,178,104,129,88,2,154,162,101,361,343):raise Unsupported(self.source,self.pos,'supported current union tag',tag,'union-tag')
        self.take(width,'union-tag')
        if self.peek()==255:self.take(1,'null-wrapper');return
        self.header({201:8,118:5,236:10,80:7,287:8,180:13,86:8,146:19,87:8,91:6,60:10,120:9,178:18,104:5,129:9,88:8,2:12,154:11,162:18,101:8,361:38,343:11}[tag])
        self.take(1,'anonymous-nonzero-byte')
