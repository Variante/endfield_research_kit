...
class Reader:
    ...
    def _action(self,depth,tag,width):
        ...
        if tag in (411,171,349):
            for _ in range(2):self.take(1,'anonymous-nonzero-byte')
            self.paired_payload();self.scalar_payload()
            # Shelter ends at its second target; the header14 actions require a DWORD.
            for _ in range(max(0,self.count(1,reserve=4 if tag==349 else 8,nullable=True))):self.keyword_edit_profile()
            self.take(1,'anonymous-nonzero-byte');self.scalar_payload();self.target_profile();self.target_profile()
            if tag!=349:self.take(4,'anonymous-scalar32')
            return
        if tag==199:
