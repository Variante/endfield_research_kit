...
class Reader:
    ...
    def _action(self,depth,tag,width):
        ...
        if tag==66:
            ...
            for _ in range(2):self.take(1,'anonymous-nonzero-byte')
            self.target_profile();self.target_profile();return
        if tag in (93,6):self.take(4,'anonymous-scalar32');return
        if tag==115:return
        if tag==333:
