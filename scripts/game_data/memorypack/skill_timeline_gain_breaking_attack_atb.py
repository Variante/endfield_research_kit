"""Selected-build SkillData GainBreakingAttackAtb 0x00BF source reader.

The native route fixes seven ordered reads, including a BlackboardDouble and
two TargetSettings fields. Nested framing remains bounded by the shared
reviewed profiles; whole-file closure belongs to the caller.
"""
...
LABEL = "skillTimelineGainBreakingAttackAtb"
CONTRACT_PATH = CONTRACTS_DIR / "skill_timeline_gain_breaking_attack_atb_native.json"
TAG = 0x00BF
READ_KINDS = (
    "bool-byte", "scalar32", "scalar32", "scalar32", "blackboard-double",
...
def _contract() -> dict[str, Any]:
    contract, _digest = read_reviewed_contract(
        CONTRACT_PATH,
        schema="endfield.skill-timeline-gain-breaking-attack-atb-native-contract.v1",
        label=LABEL,
...
def validate_current_native_contract() -> dict[str, Any]:
    """Reprove the installed route, source read order, and three nested types."""
    contract = _contract()
    ...
    for context in contract["nestedContexts"]:
        ...
        if (
            ...
            raise ValueError(f"{LABEL}.native:nested-type={index}")
    return {
        "status": "validated",
...
def decode_gain_breaking_attack_atb_action(reader: Reader, depth: int, tag: int, width: int) -> None:
    """Consume one reached seven-member action with finite nested profiles."""
    del depth
    _contract()
    ...
    reader.take(1, "GainBreakingAttackAtb.bool-byte")
    for _ in range(3):
        reader.take(4, "GainBreakingAttackAtb.scalar32")
