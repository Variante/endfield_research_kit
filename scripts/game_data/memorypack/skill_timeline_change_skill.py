"""Selected-build SkillData ChangeSkillAction 0x002C reader.
...
LABEL = "skillTimelineChangeSkill"
CONTRACT_PATH = CONTRACTS_DIR / "skill_timeline_change_skill_native.json"
TAG = 0x002C
FIELD_NAMES = (
    "isEnable", "priorityLevel", "priorityOffset", "serverActionIndex",
    "cacheTime", "duration", "inheritOriginSkillCdProgress", "lifeTimeType",
    "overrideCacheTime", "revertedSkillId", "skillSlot", "skillSource",
    "specificRevertedSkillId", "targetSkillId",
)
READ_KINDS = (
    "bool-byte", "scalar32", "scalar32", "scalar32",
    "scalar-payload", "scalar-payload", "bool-byte", "scalar32",
    "bool-byte", "byte-payload", "scalar32", "target-profile",
    "bool-byte", "byte-payload",
)
NESTED_MEMBERS = (1, 4, 5, 7, 10, 11)
NESTED_TYPES = (
    "Beyond.Gameplay.Core.AbilityAction+AbilityActionData+Priority",
    "Beyond.Blackboard+BlackboardDouble",
    "Beyond.Blackboard+BlackboardDouble",
    "Beyond.Gameplay.Core.ChangeSkillAction+LifeTimeType",
    "Beyond.Gameplay.Core.ChangeSkillAction+SkillSlot",
...
def _contract() -> dict[str, Any]:
    contract, _digest = read_reviewed_contract(
        CONTRACT_PATH,
        schema="endfield.skill-timeline-change-skill-native-contract.v1",
        label=LABEL,
...
def validate_current_native_contract() -> dict[str, Any]:
    """Authenticate dispatch, ordered reads, generic types, and child readers."""
    contract = _contract()
    ...
    for name in ("buff_ec_native.json", "buff_b2_native.json"):
...
def decode_change_skill_action(reader: Reader, depth: int, tag: int, width: int) -> None:
    """Consume one finite fourteen-member action with distinct child states."""
    del depth
    _contract()
    ...
    reader.take(1, "ChangeSkillAction.bool-byte")
    for _ in range(3):
        reader.take(4, "ChangeSkillAction.scalar32")
