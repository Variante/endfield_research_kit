...
LABEL = "skillTimelineUltimateShow"
CONTRACT_PATH = CONTRACTS_DIR / "skill_timeline_ultimate_show_native.json"
TAG = 0x0193
...
def validate_current_native_contract() -> dict[str, Any]:
    """Reprove the selected four-read source body and Priority argument."""
    contract = _contract()
    ...
    if image.setter_methods(owner, parameter="typeName", label=LABEL) != []:
        raise ValueError(f"{LABEL}.native:extra-setter")
    previous = -1
...
def decode_ultimate_show_action(reader: Reader, depth: int, tag: int, width: int) -> None:
    """Consume one reached four-member UltimateShowAction."""
    del depth
    _contract()
    ...
    if reader.peek() == 0xFF:
        ...
        return
    reader.header(4)
    reader.take(1, "UltimateShowAction.isEnable.byte")
    for field in ("priorityLevel", "priorityOffset", "serverActionIndex"):
        reader.take(4, f"UltimateShowAction.{field}.raw4")
