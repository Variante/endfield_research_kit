"""Selected-build SkillData ModifyWeaponMountPoint 0x00F3 reader.
...
LABEL = "skillTimelineModifyWeaponMountPoint"
CONTRACT_PATH = CONTRACTS_DIR / "skill_timeline_modify_weapon_mount_point_native.json"
TAG = 0x00F3
FIELD_NAMES = (
    "isEnable", "priorityLevel", "priorityOffset", "serverActionIndex",
    "fightMountPoint", "idleMountPoint", "overrideFightMountPoint",
    "overrideIdleMountPoint", "weaponIndex",
)
READ_KINDS = (
    "bool-byte", "scalar32", "scalar32", "scalar32", "scalar32", "scalar32",
    "bool-byte", "bool-byte", "scalar32",
)
...
def validate_current_native_contract() -> dict[str, Any]:
    """Authenticate selected dispatch and nine ordered source calls."""
    contract = _contract()
...
def decode_modify_weapon_mount_point_action(
    reader: Reader, depth: int, tag: int, width: int
) -> None:
    """Consume one finite nine-member 0x00F3 action."""
    del depth
    _contract()
    ...
    reader.take(1, "ModifyWeaponMountPoint.bool-byte")
    for _ in range(5):
        reader.take(4, "ModifyWeaponMountPoint.scalar32")
    reader.take(1, "ModifyWeaponMountPoint.bool-byte")
    reader.take(1, "ModifyWeaponMountPoint.bool-byte")
    reader.take(4, "ModifyWeaponMountPoint.scalar32")
