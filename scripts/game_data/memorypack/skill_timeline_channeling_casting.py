...
LABEL = "skillTimelineChannelingCasting"
CONTRACT_PATH = CONTRACTS_DIR / "skill_timeline_channeling_casting_native.json"
TAG = 0x0031
READ_KINDS = (
    "bool-byte", "enum32", "scalar32", "scalar32",
    "bool-byte", "bool-byte", "bool-byte", "scalar-payload",
)
...
def _contract() -> dict[str, Any]:
    ...
    if (
        ...
        or [row.get("memberIndex") for row in setter_calls] != [4, 5, 6, 7]
        or [row.get("methodIndex") for row in setter_calls] != [row[0] for row in setters]
        or not isinstance(contexts, list)
        or [row.get("memberIndex") for row in contexts] != [1, 7]
    ...
    if (
        ...
        != ["System.Boolean", "System.Boolean", "System.Boolean", "Beyond.Blackboard+BlackboardDouble"]
    ):
...
def validate_current_native_contract() -> dict[str, Any]:
    """Reprove the selected route, eight source calls and nested scalar."""
    contract = _contract()
...
def decode_channeling_casting_action(reader: Reader, depth: int, tag: int, width: int) -> None:
    """Consume one reached ChannelingCastingAction with bounded duration."""
    del depth
    _contract()
    ...
    if reader.peek() == 0xFF:
        ...
        return
    reader.header(8)
    reader.take(1, "ChannelingCastingAction.isEnable.byte")
    for field in ("priorityLevel", "priorityOffset", "serverActionIndex"):
        reader.take(4, f"ChannelingCastingAction.{field}.raw4")
    for field in ("cantCastSkill", "cantSwitchPosition", "cantSwitchToCenter"):
        reader.take(1, f"ChannelingCastingAction.{field}.byte")
