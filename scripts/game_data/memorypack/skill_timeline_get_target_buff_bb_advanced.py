"""Selected-build SkillData GetTargetBuffBBAdvanced 0x00C4 reader.

The selected wrapper reads two opaque string payloads around independent
BuffFindSettings and TargetSettings objects. Child readers retain their
existing finite stop behavior.
"""
...
LABEL = "skillTimelineGetTargetBuffBBAdvanced"
CONTRACT_PATH = CONTRACTS_DIR / "skill_timeline_get_target_buff_bb_advanced_native.json"
TAG = 0x00C4
FIELD_NAMES = (
    "isEnable", "priorityLevel", "priorityOffset", "serverActionIndex",
    "blackboardKey", "buffSettings", "desiredKey", "targetSettings",
)
READ_KINDS = (
    "bool-byte", "scalar32", "scalar32", "scalar32", "byte-payload",
    "finder-profile", "byte-payload", "target-profile",
)
NESTED_TYPES = (
...
def _contract() -> dict[str, Any]:
    contract, _digest = read_reviewed_contract(
        CONTRACT_PATH,
        schema="endfield.skill-timeline-get-target-buff-bb-advanced-native-contract.v1",
        label=LABEL,
    ...
    if (
        ...
        or [row.get("memberIndex") for row in contexts] != [1, 5, 7]
...
def validate_current_native_contract() -> dict[str, Any]:
    """Authenticate the selected route, ordered calls, and child contexts."""
    contract = _contract()
    ...
    for name in ("buff_b4_native.json", "buff_ec_native.json"):
...
def decode_get_target_buff_bb_advanced_action(
    reader: Reader, depth: int, tag: int, width: int
) -> None:
    """Consume one finite eight-member action with independent child states."""
    del depth
    _contract()
    ...
    reader.take(1, "GetTargetBuffBBAdvanced.bool-byte")
    for _ in range(3):
        reader.take(4, "GetTargetBuffBBAdvanced.scalar32")
    reader.byte_payload()
