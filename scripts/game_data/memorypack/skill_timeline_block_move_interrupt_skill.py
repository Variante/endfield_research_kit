...
LABEL = "skillTimelineBlockMoveInterruptSkill"
CONTRACT_PATH = CONTRACTS_DIR / "skill_timeline_block_move_interrupt_skill_native.json"
TAG = 0x001A
WRAPPER_NAME = "Beyond.MemoryPack.Beyond_Gameplay_Core_BlockMoveInterruptSkill_DataForMemoryPack"
TYPE_NAME = "Beyond.Gameplay.Core.BlockMoveInterruptSkill+Data"
...
def _contract() -> dict[str, Any]:
    contract, _digest = read_reviewed_contract(
        CONTRACT_PATH,
        schema="endfield.skill-timeline-block-move-interrupt-skill-native-contract.v1",
        label=LABEL,
...
def decode_block_move_interrupt_skill_action(reader: Reader, depth: int, tag: int, width: int) -> None:
    """Consume one finite action without inferring runtime interruption."""
    del depth
    _contract()
    ...
    reader.take(1, "BlockMoveInterruptSkill.isEnable.byte")
    for field in FIELD_NAMES[1:]:
        reader.take(4, f"BlockMoveInterruptSkill.{field}.raw4")
