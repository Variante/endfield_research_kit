...
from scripts.common import story_root_playback_aliases
from scripts.story_builder.animestudio_story_objects import reverse as audit
...
class AnimeStudioStoryReversePPtrAuditTests(unittest.TestCase):
    ...
    def test_fresh_playback_alias_loader_fails_closed_on_stage_drift(
        ...
        with tempfile.TemporaryDirectory() as temp_name:
                ...
                json.dumps({
                    ...
                    "directorHosts": [{
                        "crossStoryPlaybackAliases": [{
                            "rootStoryKey": root,
                            "playableAssetStoryKey": playable,
                            "relation":
                            ...
                            ("cutscene_f1m9d3_1", "cutscene_f1m9d4_1"),
                            ("cutscene_gm02m4_3", "cutscene_gm02m4_1"),
                        )],
                    }],
                }),
                encoding="utf-8",
            ...
            self.assertEqual(
                {
                    (row["rootStoryKey"], row["playableAssetStoryKey"])
                    for row in loaded
                },
                {
                    (
