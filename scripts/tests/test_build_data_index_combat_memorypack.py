...
import build_data_index as data_index


class CombatMemoryPackSchemaTests(unittest.TestCase):
    def test_current_schema_counts_and_new_fields_stay_in_sync(self) -> None:
        buff_schema = data_index.MEMORYPACK_FIELD_SCHEMAS["BuffData"]
        skill_schema = data_index.MEMORYPACK_FIELD_SCHEMAS["SkillData"]

        self.assertEqual(30, data_index.BUFF_MEMBER_COUNT)
        self.assertEqual(data_index.BUFF_MEMBER_COUNT, len(buff_schema))
        self.assertEqual("onlyUseSelfTimeDilation", buff_schema[21])
        self.assertEqual(47, data_index.SKILL_MEMBER_COUNT)
        self.assertEqual(data_index.SKILL_MEMBER_COUNT, len(skill_schema))
        self.assertEqual("aiExclusiveFrame", skill_schema[1])
        self.assertEqual("useAIExclusiveFrame", skill_schema[-1])

    def test_known_combat_categories_report_future_schema_drift(self) -> None:
        for category, actual, expected in (
            ("BuffData", 29, 30),
            ("SkillData", 45, 47),
        ):
            with self.subTest(category=category):
                decoded = data_index.decode_memorypack_json(
                    f"Json/{category}/fixture.json",
                    Path("missing-fixture.json"),
                    1,
                    bytes([actual]),
                )

                self.assertIsNotNone(decoded)
                self.assertEqual("unsupported-member-count", decoded["decoded"]["schemaStatus"])
                self.assertEqual(actual, decoded["decoded"]["memberCount"])
                self.assertEqual(expected, decoded["decoded"]["expectedMemberCount"])
    ...
    def test_buff_post_id_suffix_consumes_self_time_dilation_flag(self) -> None:
        data = b"".join((
            ...
            struct.pack("<I", data_index.MEMORYPACK_NULL_COUNT),
        ...
        decoded = data_index.read_buff_post_ignite_suffix(data, 0, {})
    ...
    def test_skill_exact_tail_consumes_ai_exclusive_frame_flag(self) -> None:
        ...
        decoded = data_index.decode_skill_post_switch_tail_at(data, 0, 0, "fixture")
    ...
    def test_current_stack_effect_action_boundary_and_nonempty_stacking_key(self) -> None:
        ...
        decoded, end = data_index.skip_buff_stack_effects_effect_actions_body(data, 0, 1)
    ...
    def test_buff_compact_tag_id_list_keeps_packed_ids(self) -> None:
        ...
        decoded, end = data_index.read_buff_compact_tag_id_list_field(data, 0, "tags")
    ...
    def test_current_energy_shard_nested_block_headers_are_accepted(self) -> None:
        tail_prefix = data_index.BUFF_IGNITE_NESTED_BLOCK_TAIL_PREFIX
        blocks = []
        for index, tail_code in enumerate((2, 3, 4, 5)):
            variants = (
                data_index.BUFF_IGNITE_NESTED_BLOCK_LONG_HEADERS
                if index < 3
                else data_index.BUFF_IGNITE_NESTED_BLOCK_SHORT_HEADERS
            )
        ...
        decoded = data_index.validate_buff_ignite_nested_blocks(data, 0, len(data), 4)
