...
from scripts.story_recovery import runtime_trace_mission_capture as capture
...
class MissionRuntimeCaptureTests(unittest.TestCase):
    def test_manifest_references_and_loads_canonical_task_paths(self):
        raw = json.loads(capture.DEFAULT_MANIFEST.read_text(encoding="utf-8"))
        self.assertNotIn("levelScriptTask", raw["hooks"])
        self.assertEqual(
            raw["missionTaskPathsContract"],
            "../story_builder/native_contracts/mission_task_paths.json",
        )

        manifest = capture.load_manifest(capture.DEFAULT_MANIFEST)
        contract = load_mission_task_paths(DEFAULT_CONTRACT)
        self.assertEqual(manifest["hooks"]["levelScriptTask"], contract["hooks"])
        self.assertEqual(manifest["missionTaskPathsContractSha256"], contract["sha256"])

    def test_manifest_refuses_embedded_task_path_duplicate(self):
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            contract_path = root / "mission_task_paths.json"
            contract_path.write_bytes(DEFAULT_CONTRACT.read_bytes())
            manifest = json.loads(
                capture.DEFAULT_MANIFEST.read_text(encoding="utf-8")
            )
            manifest["missionTaskPathsContract"] = contract_path.name
            manifest["hooks"]["levelScriptTask"] = {}
            manifest_path = root / "hooks.json"
            manifest_path.write_text(json.dumps(manifest), encoding="utf-8")

            with self.assertRaisesRegex(
                core.CaptureConfigurationError,
                r"embedded hooks\.levelScriptTask is not allowed",
            ):
                capture.load_manifest(manifest_path)

    def test_manifest_refuses_task_contract_for_another_build(self):
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            contract = json.loads(DEFAULT_CONTRACT.read_text(encoding="utf-8"))
            contract["gameBuild"] = "different-build"
            contract_path = root / "mission_task_paths.json"
            ...
            with self.assertRaisesRegex(
                core.CaptureConfigurationError,
                "targets a different game build",
            ):
                capture.load_manifest(manifest_path)
    ...
    def test_changed_file_is_refused(self):
        ...
        with self.assertRaisesRegex(core.CaptureConfigurationError, "SHA-256 changed"):
