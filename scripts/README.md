...
| | `game_data/wwise_sdk_symbols.py` | names functions in the shipped `AkSoundEngine.dll` by matching each `.pdata` function body against the installed Wwise SDK library's named COMDAT sections; a match needs an equal extent, 80% byte agreement and a clear margin over the runner-up, so an ambiguous or weak candidate leaves the function unnamed |
