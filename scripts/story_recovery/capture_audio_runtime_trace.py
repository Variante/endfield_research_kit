r"""Capture hash-locked audio runtime requests with Frida.
...
Run from the repository root with the repo-local Frida environment::
    ...
    tools\frida-runtime\venv\Scripts\python.exe \
        scripts\story_recovery\capture_audio_runtime_trace.py
...
if __package__:
    from . import capture_mission_runtime_trace as support
else:
    sys.path.insert(0, str(ROOT))
    from scripts.story_recovery import capture_mission_runtime_trace as support


DEFAULT_GAME_ROOT = support.DEFAULT_GAME_ROOT
...
def default_output_path() -> Path:
    ...
    return ROOT / "scratch" / "story" / "runtime_trace" / f"audio-runtime-{stamp}.jsonl"
...
def load_manifest(path: Path) -> dict[str, Any]:
    try:
        ...
        raise support.CaptureConfigurationError(f"audio hook manifest not found: {path}") from exc
    except json.JSONDecodeError as exc:
        raise support.CaptureConfigurationError(
            f"invalid audio hook manifest JSON: {path}: {exc}"
        ) from exc
    if not isinstance(value, dict) or value.get("schema") != MANIFEST_SCHEMA:
        raise support.CaptureConfigurationError(f"manifest schema must be {MANIFEST_SCHEMA!r}")
    for key in ("gameBuild", "processName", "moduleName"):
        if not isinstance(value.get(key), str) or not value[key].strip():
            raise support.CaptureConfigurationError(f"manifest {key} must be a non-empty string")
    files = value.get("files")
    if not isinstance(files, dict) or not files:
        raise support.CaptureConfigurationError("audio manifest must contain a non-empty files object")
    for required in ("executable", "gameAssembly", "metadata"):
        if required not in files:
            raise support.CaptureConfigurationError(f"audio manifest is missing files.{required}")
    hooks = value.get("hooks")
    if not isinstance(hooks, list) or not hooks:
        raise support.CaptureConfigurationError("audio manifest hooks must be a non-empty list")
    names: set[str] = set()
    for index, hook in enumerate(hooks):
        if not isinstance(hook, dict):
            raise support.CaptureConfigurationError(f"hooks[{index}] must be an object")
        name = hook.get("name")
        if not isinstance(name, str) or not name.strip() or name in names:
            raise support.CaptureConfigurationError(f"hooks[{index}] has a duplicate/invalid name")
        names.add(name)
        rva = hook.get("rva")
        if not isinstance(rva, str) or not rva.lower().startswith("0x"):
            raise support.CaptureConfigurationError(f"hooks[{index}] has an invalid RVA")
        try:
            if int(rva, 16) < 0:
                raise ValueError
        ...
        if not isinstance(hook.get("sourceKind"), str) or not hook["sourceKind"].strip():
            raise support.CaptureConfigurationError(f"hooks[{index}] sourceKind is required")
        if "required" in hook and not isinstance(hook["required"], bool):
            raise support.CaptureConfigurationError(f"hooks[{index}] required must be boolean")
    if not isinstance(value.get("evidenceBoundary"), dict):
        raise support.CaptureConfigurationError("audio manifest evidenceBoundary must be an object")
    return value
...
def render_agent_source(path: Path, manifest: dict[str, Any]) -> str:
    try:
        ...
        raise support.CaptureConfigurationError(f"audio Frida agent not found: {path}") from exc
    if source.count(AUDIO_AGENT_PLACEHOLDER) != 1:
        raise support.CaptureConfigurationError(
            f"audio Frida agent must contain exactly one {AUDIO_AGENT_PLACEHOLDER} placeholder"
        )
    config = {
...
class AudioEventWriter:
    def __init__(self, output: Path, session_id: str, start: float) -> None:
        self.output = output
        self.diagnostics = output.with_name(f"{output.stem}.diagnostics.jsonl")
...
def run_capture(
    ...
    frida = support.load_frida()
    process_name = args.process or manifest["processName"]
    device = frida.get_local_device()
    process = (
        support.process_from_verified_pid(device, args.pid, process_name)
        if args.pid is not None
        else support.find_process(device, process_name, args.wait_seconds)
    )
    ...
    try:
        ...
        support.wait_for_modules(session, [manifest["moduleName"]])
...
def main() -> int:
    ...
    try:
        ...
        verified = support.verify_game_files(args.game_root.resolve(), manifest)
    ...
    except (support.CaptureConfigurationError, TimeoutError, RuntimeError, KeyError) as exc:
