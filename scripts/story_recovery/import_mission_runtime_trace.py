"""Validate and normalize observed Mission/LevelScript runtime traces.
...
Run from the repository root:
    python scripts/story_recovery/import_mission_runtime_trace.py capture.jsonl
"""
...
def read_events(paths: Iterable[Path]) -> tuple[list[dict[str, Any]], list[str]]:
    ...
    for path in paths:
        ...
        if not resolved.is_file():
            raise FileNotFoundError(f"runtime trace not found: {path}")
        try:
        ...
        with resolved.open("r", encoding="utf-8") as handle:
            for line_number, line in enumerate(handle, 1):
                ...
                try:
                    ...
                    raise _fail(source, f"invalid JSON: {exc.msg}") from exc
                if not isinstance(row, dict):
                    raise _fail(source, "each JSONL row must be an object")
                events.append(normalize_event(row, source))
    if not events:
        raise TraceValidationError("runtime trace contains no events")
...
def main() -> int:
    ...
    markdown_output = (
        args.markdown_output.resolve() if args.markdown_output else output.with_suffix(".md")
    )
    write_json(output, bundle)
    markdown_output.parent.mkdir(parents=True, exist_ok=True)
    markdown_output.write_text(render_markdown(bundle), encoding="utf-8", newline="\n")
    summary = bundle["summary"]
    print(
