r"""Validate and summarize an observed audio runtime JSONL capture.
...
Run from the repository root::
    ...
    python scripts\story_recovery\import_audio_runtime_trace.py capture.jsonl
"""
...
def read_events(paths: Iterable[Path]) -> tuple[list[dict[str, Any]], list[str]]:
    ...
    for path in paths:
        ...
        if not resolved.is_file():
            raise FileNotFoundError(f"audio runtime trace not found: {path}")
        try:
            source_name = resolved.relative_to(ROOT).as_posix()
        except ValueError:
        ...
        with resolved.open("r", encoding="utf-8") as handle:
            for line_number, line in enumerate(handle, 1):
                ...
                try:
                    ...
                    raise fail(source, f"invalid JSON: {exc.msg}") from exc
                if not isinstance(row, dict):
                    raise fail(source, "each row must be an object")
                events.append(normalize_event(row, source))
    if not events:
        raise AudioTraceValidationError("audio runtime trace contains no events")
...
def main() -> int:
    ...
    try:
        ...
        markdown_output.write_text(markdown(bundle), encoding="utf-8")
        print(f"wrote JSON: {output}")
        print(f"wrote Markdown: {markdown_output}")
        return 0
    except (AudioTraceValidationError, FileNotFoundError, OSError) as exc:
        print(f"Audio runtime trace import failed: {exc}", file=sys.stderr)
        return 1
