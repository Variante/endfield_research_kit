...
def build_tasks(args: argparse.Namespace) -> list[TaskSpec]:
    ...
    tasks.append(
        TaskSpec(
            "map_recovery",
            (module("scripts.webui.map.build_map_recovery_data", "--jobs", map_recovery_jobs),),
        )
    )

    if args.with_assets:
        # scripts/webui/assets/build_assets.py scans export_full and writes
        # webui/data/assets/{index,story_media}.json. No in-runner input.
        tasks.append(
            TaskSpec(
                "assets",
                (module("scripts.webui.assets.build_assets", "--mode", args.asset_mode),),
            )
        )
    ...
    tasks.append(
        TaskSpec(
            ...
            (
                module(
                    "scripts.webui.characters.build_character_data",
                    "--languages",
                    "CN",
                    "--default-language",
                    "CN",
    ...
    tasks.append(
        TaskSpec(
            ...
            (
                module(
                    "scripts.webui.map.build_map_recovery_data",
                    "--preview-only",
                    "--jobs",
                    map_recovery_jobs,
                ),
