...
from scripts.webui.story.level_bindings import (
    ...
    _build_levelscript_file_order_scene_sequences,
    _build_levelscript_scene_chain_map,
    _load_leveldata_named_entries,
    _load_levelscript_binding_data,
    _load_npc_proxy_ex,
    _scene_binding_search_text,
    build_entity_tracking_native_event_story_context,
    ...
    build_leveldata_mission_script_host_index,
    ...
    build_levelscript_mission_state_story_routes,
    build_levelscript_native_black_action_index,
    ...
    build_mission_scene_file_order,
    ...
    infer_mission_dialog_order,
    match_entity_tracking_native_entity_event_context,
    match_mission_area_leader_trigger_context,
...
def build_language_bundle(
    ...
    *,
    ...
    def pack_options(
        ...
        for order, g in enumerate(sorted(groups_map), start=1):
            ...
            if after_is_authored and after:
                ...
                group["after"] = fallback_anchor_id
            timeline_route_branch = timeline_route_branch_for_group(
                group_opt_ids,
                group.get("after") or "",
                valid_line_ids=valid_line_ids,
                tree_branches=tree_branches,
                timeline_after=timeline_after,
                timeline_pre=timeline_pre,
                timeline_option_routes=timeline_option_routes,
                local_ordered_line_ids=local_ordered_line_ids,
            ...
            if not any(opt.get("branchLines") for opt in opts):
                dialog_tree_node_layout = (
                    dialog_tree_option_node_layout_for_group(
                        group_opt_ids,
                        group.get("after") or "",
                        tree_option_node_layouts=tree_option_node_layouts,
                    )
                )
            following_line_risk = (
                ...
                or following_line_risk_for_group(
                    group_opt_ids,
                    group.get("after") or "",
                    valid_line_ids=valid_line_ids,
                    tree_branches=tree_branches,
                    tree_converge=tree_converge,
                    timeline_option_rows=timeline_option_rows,
                    timeline_after=timeline_after,
                    timeline_pre=timeline_pre,
                    timeline_after_line_ids=timeline_after_line_ids,
                    timeline_after_line_timings=timeline_after_line_timings,
    ...
    for key, items in groups.items():
        ...
        scene_graph_links = apply_source_hub_option_groups(
            payload,
            scene_graph_links,
            _load_dialog_tree_source(out_key),
            option_payload_by_id=dialog_option_payload_by_id,
            option_signature_by_id=dialog_option_signature_by_id,
            option_group_parts=_option_id_group_parts,
            unique_preserve=_unique_preserve,
    ...
    cutscene_text_by_key = project_cutscene_text_lines(
        set(cutscene_assets),
        _load_cutscene_subtitle_tracks(),
        inputs=CutsceneTextInputs(
            text_table=text_table,
            language_code=language_code,
            subtitle_parent_overrides=cutscene_subtitle_parent_overrides,
            cutscene_text_row_re=CUTSCENE_TEXT_ROW_RE,
        ),
        callbacks=CutsceneTextCallbacks(
            split_parent_key=split_cutscene_parent_key,
            split_child_sort_key=split_cutscene_child_sort_key,
            resolve_text_group=resolve_cutscene_text_group,
            translate=t,
            remember_texttable_row_usage=remember_texttable_row_usage,
            source_ref=source_ref,
            pick_fields=pick_fields,
            text_trace=text_trace,
            build_texttable_line=build_cutscene_texttable_line,
            select_display_text_group=select_subtitle_text_group_from_display_names,
            ...
            pair_normalize=cutscene_pair_normalize,
        ),
    )
    ...
    def write_texttable_collection_pages(excluded_row_ids: set[str] | None = None) -> None:
        ...
        for bucket, entries in sorted(chunks_by_bucket.items()):
            ...
            story_ref = collection_story_ref_from_bucket(
                bucket,
                parse_mission_id=parse_mission,
                mission_story_types=MISSION_STORY_TYPES,
            )
        ...
        for table_path in generic_collection_paths:
            ...
            for row_index, (row_id, row) in enumerate(sorted(payload.items(), key=lambda item: str(item[0])), start=1):
                ...
                story_ref = collection_reading_story_ref(
                    ...
                    row_key,
                    row if isinstance(row, dict) else None,
                ) or collection_story_ref_from_bucket(
                    bucket,
                    parse_mission_id=parse_mission,
                    mission_story_types=MISSION_STORY_TYPES,
                )
                ...
                linked_content_refs, linked_preview_text = append_linked_reading_content_lines(
                    ...
                    fallback_rich_content=rich_content,
                    radio_row_lookup=radio_row_lookup,
                    translate=t,
                    source_ref=source_ref,
                    pick_fields=pick_fields,
                    text_trace=text_trace,
    ...
    def attach_narrative_videos_to_outputs() -> dict:
        ...
        standalone_entries = emit_standalone_video_outputs(
            standalone_videos,
            narrative_video_name_sort_key=narrative_video_name_sort_key,
            compact_narrative_video_ref=compact_narrative_video_ref,
            parse_mission=parse_mission,
            unique_preserve=_unique_preserve,
            mission_name_trace=mission_name_trace,
            write_conv_payload=write_conv_payload,
            narrative_video_index_summary=narrative_video_index_summary,
            preview=preview,
            merge_search_text=merge_search_text,
            mission_context_text=mission_context_text,
            text_table=text_table,
            translate=t,
            source_ref=source_ref,
            pick_fields=pick_fields,
    ...
    for (story_key, dialog_key), occurrence_rows in sorted(
        ...
        connection = dialog_tree_story_playback_connection(
            ...
            story_kind_by_key=story_kind_by_key,
            story_owner_by_key=story_owner_by_key,
        )
    ...
    def emit_webui_secret_notice() -> None:
        ...
        provenance = {
            ...
            "sourceFile": "scripts/webui/story/language_bundle.py",
