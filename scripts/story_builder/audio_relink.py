...
def relink_existing_audio(target_languages: list[str]) -> None:
    ...
    for language_code in target_languages:
        ...
        audio_root = Path(getattr(audio_builder, "DEFAULT_AUDIO_ROOT"))
        language_root = audio_root / language_code
        has_decoded_audio = getattr(audio_builder, "has_decoded_audio", None)
        has_audio = (
            has_decoded_audio(language_root)
            if callable(has_decoded_audio)
            else language_root.exists()
        )
        if not has_audio:
            print(f"Audio relink [{language_code}]: skipped (no decoded audio files at {language_root})")
            continue

        print(f"\nAudio relink [{language_code}]: using existing decoded audio")
        audio_args = audio_builder.parse_args([
            "--language",
            language_code,
            "--skip-decode",
            "--audio-root",
