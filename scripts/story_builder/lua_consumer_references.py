#!/usr/bin/env python3
"""Build the canonical Lua consumer-reference index used by Story builders.

The VFS Lua block is already decrypted into plaintext under the structured
export roots. This module turns those scripts into a fingerprinted builder
index.  Mission Pipeline imports it directly; the command-line entry point can
also render the optional human-readable evidence report.
...
def build_index(
    *,
    ...
    focus_example_limit: int = 4,
    max_focus_files: int = 30,
    max_module_findings: int = 300,
    max_table_module_edges: int = 2500,
) -> dict[str, Any]:
    """Scan current sources and return the canonical fingerprinted index."""
    return _build_payload(
        argparse.Namespace(
            ...
            focus_example_limit=focus_example_limit,
            max_focus_files=max_focus_files,
            max_module_findings=max_module_findings,
            max_table_module_edges=max_table_module_edges,
        )
    )
...
def write_index(
    ...
    **scan_options: Any,
    ...
    """Refresh the canonical index without timestamp-only hash churn."""
    payload = build_index(**scan_options)
    if output_path.is_file():
        ...
        if isinstance(previous, dict):
            previous_semantics = dict(previous)
            current_semantics = dict(payload)
            previous_semantics.pop("generatedAt", None)
            current_semantics.pop("generatedAt", None)
            if previous_semantics == current_semantics and previous.get("generatedAt"):
                payload["generatedAt"] = previous["generatedAt"]
    write_report_json(output_path, payload)
    if markdown_path is not None:
        write_text_if_changed(markdown_path, render_markdown(payload))
    return payload


def read_index(path: Path = DEFAULT_INDEX) -> dict[str, Any]:
    """Read the builder-owned canonical index without recovery-module coupling."""
    try:
        payload = json.loads(path.read_text(encoding="utf-8"))
    except OSError as exc:
        raise RuntimeError(f"unable to read Lua consumer index {repo_rel(path)}: {exc}") from exc
    except json.JSONDecodeError as exc:
        raise RuntimeError(f"invalid Lua consumer index {repo_rel(path)}: {exc}") from exc
    if not isinstance(payload, dict):
        raise RuntimeError(
            f"invalid Lua consumer index {repo_rel(path)}: expected JSON object"
        )
    return payload
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help="canonical JSON index path",
    )
    parser.add_argument(
        "--markdown",
        type=Path,
        nargs="?",
        const=DEFAULT_MD,
        help="optionally render Markdown (default path when flag has no value)",
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    payload = write_index(
        ...
        focus_example_limit=args.focus_example_limit,
        max_focus_files=args.max_focus_files,
        max_module_findings=args.max_module_findings,
        max_table_module_edges=args.max_table_module_edges,
    )
    summary = payload["summary"]
    print(f"Lua consumer reference index: {args.output}")
