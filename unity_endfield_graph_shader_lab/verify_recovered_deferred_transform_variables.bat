...
set "METHOD=EndfieldGraphShaderLabEditor.EndfieldRecoveredDeferredTransformVariablesBatchVerifier.VerifyBatch"
set "FRAME_METHOD=EndfieldGraphShaderLabEditor.EndfieldManifestCharacterSetup.RenderRuntimeReferenceWulfaPreview"
set "OUTPUT_DIR=%PROJECT_PATH%\scratch\character_recovery\deferred_transform_variables"
set "MODE=%~1"
...
python "%PROJECT_PATH%\tools\audit_deferred_transform_variables.py" --check
...
if errorlevel 1 exit /b !ERRORLEVEL!
python "%PROJECT_PATH%\tools\verify_deferred_transform_variables.py" --root "%OUTPUT_DIR%"
exit /b !ERRORLEVEL!

:run_api
set "API=%~1"
set "LOG_PATH=%OUTPUT_DIR%\unity_validation_%API%.log"
echo Validating recovered deferred TransformVariables with %API%...
...
echo Validating same-frame deferred TransformVariables with %API%...
...
echo Validating fail-closed deferred TransformVariables with %API%...
"%UNITY_EXE%" -batchmode -quit -force-%API% -projectPath "%PROJECT_PATH%" -executeMethod %FRAME_METHOD% -logFile "%LOG_PATH%"
set "RUN_EXIT=%ERRORLEVEL%"
if not "%RUN_EXIT%"=="0" exit /b %RUN_EXIT%
copy /y "%PROJECT_PATH%\..\scratch\runtime_reference_wulfa.png" "%BEAUTY_PATH%" >nul
