...
set "GPU_METHOD=EndfieldGraphShaderLabEditor.EndfieldRecoveredDeferredShadowDataBatchVerifier.VerifyBatch"
set "FRAME_METHOD=EndfieldGraphShaderLabEditor.EndfieldManifestCharacterSetup.RenderRuntimeReferenceWulfaPreview"
set "ZHUANGFY_FRAME_METHOD=EndfieldGraphShaderLabEditor.EndfieldManifestCharacterSetup.RenderRuntimeReferenceZhuangfyPreview"
set "OUTPUT_DIR=%PROJECT_PATH%\scratch\character_recovery\deferred_shadow_data"
...
set "ENDFIELD_RECOVERED_SEPARATE_CHARACTER_SHADOW=0"
...
python "%PROJECT_PATH%\tools\audit_deferred_shadow_data.py" --check
...
call :run_gpu d3d12
if errorlevel 1 exit /b !ERRORLEVEL!
call :run_frame d3d11 active
if errorlevel 1 exit /b !ERRORLEVEL!
call :run_frame d3d12 active
if errorlevel 1 exit /b !ERRORLEVEL!
call :run_zhuangfy_frame d3d12
if errorlevel 1 exit /b !ERRORLEVEL!
set "ENDFIELD_RECOVERED_DEFERRED_SHADOW_DATA=0"
call :run_frame d3d11 control
if errorlevel 1 exit /b !ERRORLEVEL!
call :run_frame d3d12 control
if errorlevel 1 exit /b !ERRORLEVEL!
set "ENDFIELD_RECOVERED_DEFERRED_SHADOW_DATA=1"
set "ENDFIELD_RECOVERED_ISOLATED_PUNCTUAL_SOFT_SHADOWS=0"
call :run_fail_closed d3d12
if errorlevel 1 exit /b !ERRORLEVEL!
python "%PROJECT_PATH%\tools\verify_deferred_shadow_data.py" --root "%OUTPUT_DIR%"
exit /b !ERRORLEVEL!

:run_gpu
set "API=%~1"
"%UNITY_EXE%" -batchmode -quit -force-%API% -projectPath "%PROJECT_PATH%" -executeMethod %GPU_METHOD% -logFile "%OUTPUT_DIR%\unity_validation_%API%.log"
exit /b %ERRORLEVEL%
...
set "MODE=%~2"
if /I "%MODE%"=="active" (
  set "LOG_NAME=unity_frame_%API%.log"
) else (
  set "LOG_NAME=unity_control_%API%.log"
)
"%UNITY_EXE%" -batchmode -quit -force-%API% -projectPath "%PROJECT_PATH%" -executeMethod %FRAME_METHOD% -logFile "%OUTPUT_DIR%\!LOG_NAME!"
if errorlevel 1 exit /b !ERRORLEVEL!
if /I "%MODE%"=="active" (
  copy /y "%PROJECT_PATH%\..\scratch\runtime_reference_wulfa.png" "%OUTPUT_DIR%\wulfa_beauty_%API%.png" >nul
) else (
  copy /y "%PROJECT_PATH%\..\scratch\runtime_reference_wulfa.png" "%OUTPUT_DIR%\wulfa_beauty_control_%API%.png" >nul
)
exit /b %ERRORLEVEL%

:run_zhuangfy_frame
set "API=%~1"
"%UNITY_EXE%" -batchmode -quit -force-%API% -projectPath "%PROJECT_PATH%" -executeMethod %ZHUANGFY_FRAME_METHOD% -logFile "%OUTPUT_DIR%\unity_zhuangfy_%API%.log"
if errorlevel 1 exit /b !ERRORLEVEL!
copy /y "%PROJECT_PATH%\..\scratch\runtime_reference_zhuangfy.png" "%OUTPUT_DIR%\zhuangfy_beauty_%API%.png" >nul
exit /b %ERRORLEVEL%
