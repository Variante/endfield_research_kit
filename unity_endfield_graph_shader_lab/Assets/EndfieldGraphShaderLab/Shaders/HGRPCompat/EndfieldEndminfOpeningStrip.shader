Shader "Hidden/Endfield/HGRPCompat/EndminfOpeningStrip"
{
    ...
    SubShader
    {
        ...
        void EvaluateOpeningStrip(
            float2 uv,
            out float2 shiftedUv,
            out float chromaUv,
            out float coverage)
        {
            ...
            chromaUv = activeShift.y *
                (_EndminfOpeningStripSourceSize.x / 1920.0) *
                _EndminfOpeningStripSourceSize.z;
            shiftedUv = float2(saturate(uv.x - displacementUv), uv.y);

            // GBufferA is cleared to zero and receives selector bits only
            // where the current CharacterPrePass owns a pixel. Sample it at
            // the displaced coordinate so copied bands may enter static space.
            ...
            float destinationOwner = step(
                1e-5,
                dot(abs(destinationSelector), float4(1.0, 1.0, 1.0, 1.0)));
            coverage = saturate(_EndminfOpeningStripParams.x) *
                activeBand * shiftedOwner * (1.0 - destinationOwner);
        }

        float4 FragColor(v2f_img input) : SV_Target
        {
            float2 shiftedUv;
            float chromaUv;
            float coverage;
            EvaluateOpeningStrip(input.uv, shiftedUv, chromaUv, coverage);
            float4 shifted = float4(
                tex2Dlod(_MainTex, float4(saturate(shiftedUv + float2(chromaUv, 0.0)), 0.0, 0.0)).r,
                tex2Dlod(_MainTex, float4(shiftedUv, 0.0, 0.0)).g,
                tex2Dlod(_MainTex, float4(saturate(shiftedUv - float2(chromaUv, 0.0)), 0.0, 0.0)).b,
                tex2Dlod(_MainTex, float4(shiftedUv, 0.0, 0.0)).a);
            float4 original = tex2Dlod(
                _MainTex, float4(input.uv, 0.0, 0.0));
            return lerp(original, shifted, coverage);
        }

        float4 FragSceneMV(v2f_img input) : SV_Target
        {
            float2 shiftedUv;
            float chromaUv;
            float coverage;
            ...
            float4 original = tex2Dlod(
                _MainTex, float4(input.uv, 0.0, 0.0));
            // Retail Target1 uses SrcColor/InvSrcColor for RGB and One/One
            // for alpha with source (0,0,1,0): preserve XY/A and set B=1.
            original.b = max(original.b, coverage);
            return original;
        }
        ENDCG
