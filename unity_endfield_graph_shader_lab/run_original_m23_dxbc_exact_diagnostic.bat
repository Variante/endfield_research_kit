...
del /F /Q "%PLAYER_REPORT%" "%D3D12_REPORT%" "%OUTPUT_DIR%\standalone_build.json" 2>nul
...
"%PLAYER_EXE%" -batchmode -force-d3d11 -screen-width 1 -screen-height 1 -endfield-original-m23-dxbc-diagnostic -endfield-original-m23-dxbc-output "%PLAYER_REPORT%" -logFile "%PLAYER_LOG%"
...
python "%PROJECT_PATH%\tools\test_original_m23_dxbc_exact_managed.py" --report "%PLAYER_REPORT%" --d3d12-report "%D3D12_REPORT%"
