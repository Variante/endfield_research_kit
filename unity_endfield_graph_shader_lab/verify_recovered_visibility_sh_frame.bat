...
set "OUTPUT_ROOT=%PROJECT_PATH%\scratch\character_recovery\visibility_sh_frame"
...
echo Usage: %~nx0 --all ^| --d3d11 ^| --d3d12 ^| --fail-closed-d3d12
exit /b 2
...
echo Validating same-frame canonical VisibilitySH publication with !API!...
...
python "%PROJECT_PATH%\tools\verify_visibility_sh_frame_log.py" --api !API! --log "!LOG_PATH!" --report "!REPORT_PATH!" --beauty "!BEAUTY_PATH!"
...
echo Validating fail-closed canonical VisibilitySH publication with !API!...
"%UNITY_EXE%" -batchmode -quit -projectPath "%PROJECT_PATH%" -force-!API! -executeMethod %METHOD% -logFile "!LOG_PATH!"
set "UNITY_EXIT=!ERRORLEVEL!"
if not "!UNITY_EXIT!"=="0" exit /b !UNITY_EXIT!
copy /y "%PROJECT_PATH%\..\scratch\runtime_reference_wulfa.png" "!BEAUTY_PATH!" >nul
set "COPY_EXIT=!ERRORLEVEL!"
if not "!COPY_EXIT!"=="0" exit /b !COPY_EXIT!
python "%PROJECT_PATH%\tools\verify_visibility_sh_frame_log.py" --api !API! --log "!LOG_PATH!" --report "!REPORT_PATH!" --beauty "!BEAUTY_PATH!" --expect-fail-closed
set "VERIFY_EXIT=!ERRORLEVEL!"
exit /b !VERIFY_EXIT!
