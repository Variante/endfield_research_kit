...
class BaselineTests(unittest.TestCase):
    def test_contract_is_sparse_and_explicit(self):
        r=m.verify(); self.assertEqual(r['status'],'sparse_unpaired_baseline'); self.assertEqual(r['unityPhaseRenders']['missingPhases'],['start','transition']); self.assertEqual(len(r['comparisons']),3); self.assertTrue(all(x['phaseMapping'].startswith('explicitly unpaired') for x in r['comparisons']))
