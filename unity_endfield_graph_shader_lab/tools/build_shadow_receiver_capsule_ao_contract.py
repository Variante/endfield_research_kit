...
def build_contract(disassembly: str) -> dict:
    ...
    return {
        ...
        "algorithm": (
            "Spherical-harmonic exponentiation of a log-space visibility SH. "
            "The buffer holds accumulated log occlusion; the receiver evaluates "
            "exp(SH) as (exp(SH / 2^n))^(2^n), which is why it halves until the "
            "linear magnitude falls under the threshold, applies the LUT once, "
            "and then repeats the SH self-product exactly n times."
        ),
        "constants": {
            "sh_band0": {
                "value": 0.282095,
                "derivation": "Y00 = 1/(2*sqrt(pi)) = 0.28209479177387814",
            },
            "sh_band1_convolved": {
                "value": -0.325735,
                "derivation": (
                    "-(2/3)*sqrt(3/(4*pi)) = -0.32573500793527993, the band-1 "
                    "irradiance convolution weight"
                ),
            },
            "band0_gain": {
                "value": 3.544908,
                "derivation": "2*sqrt(pi) = 3.5449077018110318, the inverse of Y00",
            },
            "log_exponent_scale": {
                "value": 0.406977,
                "derivation": (
                    "Y00 * log2(e) = 0.40697675715277876. The DXBC exp opcode is "
                    "base 2, so exp2(sh.x * 0.406977) is exactly exp(Y00 * sh.x), "
                    "the DC term of the SH exponentiation"
                ),
            },
        ...
        "cb4_3_constraint": (
            "cb4[3].xy maps the post-normalisation magnitude length(sh.yzw) into "
            "the LUT's [0,1] domain, since the result is quantised as "
            "(x*255+0.5)/256. The halving loop bounds that magnitude at the 4.6 "
            "threshold, so a scale of 1/4.6 with zero bias is consistent with the "
            "observed domain. That is a consistency bound, not a recovered value, "
            "and must not be substituted for one."
        ),
        "landmarks": verify_landmarks(disassembly),
        "ruled_out_name_sources": [
            {
                "source": "fragment DXBC reflection chunk",
                "result": "absent",
                "detail": (
                    "the container holds only ISGN, OSGN and SHEX; there is no "
                    "RDEF chunk, so no constant-buffer or variable names"
                ),
            },
            {
                "source": (
                    "AnimeStudio HLSL decompile of the source Shader asset "
                    "HGRP_CharacterNPR_ShadowReceiver_p22FE8453732C0E54.shader"
                ),
                "result": "unnamed",
                "detail": (
                    "constant buffers are emitted as unnamed arrays such as "
                    "float4 cb4[715]; register assignment also differs per "
                    "variant, so slot numbers are not comparable across them"
                ),
            },
            {
                "source": "Vulkan SMOL-V subprogram in the same Shader asset",
                "result": "unusable",
                "detail": (
                    "no OpName or OpMemberName debug records, and the decoded "
                    "decorations are visibly corrupt: every OpMemberDecorate "
                    "Offset reads 10 and DescriptorSet/Binding pairs mirror "
                    "their own result ids"
                ),
            },
        ],
        "next_source_to_try": (
            "the installed GameAssembly VisibilitySH pass setup, which writes "
            "this constant buffer, rather than any shader asset"
        ),
        "open_gap": (
            "None. Every constant, both LUT identities, and the channel reads are "
            "source-closed offline, the term is implemented, and the capsule set "
            "is verified source-exact. The only unconsumed retail data on this "
            "path is the isFoot flag on the two foot capsules."
        ),
        "independent_confirmation": {
            "shader": "HGRP/VisibilitySH",
            "export": "HGRP_VisibilitySH_pC373B6179F491022.shader",
            "finding": (
                "That shader's own resolve pass performs the identical SH "
                "exponentiation with a four-vector cb0 using indices 2 and 3, "
                "confirming the receiver's cb4 reads from a second, unrelated "
                "program"
            ),
            "matching_details": [
                "halving threshold 4.5999999",
                "coordinate quantisation x*255+0.5 then *0.00390625",
                "LUT sampled .xy, which independently confirms the channel read",
                "cb0[3].xy encode scale/bias and cb0[2].xy/.zw coefficient scale/bias",
                "full-precision constants 3.54490781, 0.40697673, 0.282094777",
                "the same four-dot self-product loop shape",
            ],
        },
        "capsule_set_validation": {
            "result": "the lab capsule set is source-exact",
            "detail": (
                "Retail serialises the capsules as HGCapsuleShadowHelper "
                "m_capsuleShadowContainers, ten per actor. Wulfa matches the lab "
                "on every height, radius, intensity, offset and rotation to a "
                "worst absolute delta of 5e-15; Zhuangfy matches to about 1e-7 "
                "except one upper-arm capsule at 0.162 degrees, and retail has "
                "twenty variants carrying that signature, so that is variant "
                "selection rather than an error."
            ),
            "unconsumed_retail_fields": ["isFoot", "enabled"],
            "note": (
                "isFoot is set on exactly the two foot capsules and the lab "
                "ignores it. The retail helper also binds fourteen skeleton "
                "joints including Wrist_R/L and ToesEnd_R/L, which are capsule "
                "endpoints rather than additional capsules; the ten-capsule count "
                "is correct."
            ),
        },
