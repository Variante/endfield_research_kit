...
def _serialize_report(report: dict[str, Any]) -> bytes:
    """Return the one canonical byte representation published by this tool."""
...
def _write_report(report: dict[str, Any]) -> None:
    ...
    # Pin newline handling so --check is portable and generated bytes do not
    # change solely because the command was run on Windows versus POSIX.
...
def _check_report(report: dict[str, Any]) -> None:
    ...
    try:
        ...
        raise PayloadDecodeError(f"check: missing generated report {OUTPUT}") from exc
    expected = _serialize_report(report)
    if actual != expected:
        raise PayloadDecodeError(
            f"check: generated report is stale or non-canonical {OUTPUT} "
...
def main(argv: list[str] | None = None) -> int:
    ...
    except (OSError, PayloadDecodeError, json.JSONDecodeError) as exc:
