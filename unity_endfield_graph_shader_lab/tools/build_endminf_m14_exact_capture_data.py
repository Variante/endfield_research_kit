...
def render_cpp(packets: list[dict[str, Any]]) -> str:
    ...
    for packet_index, packet in enumerate(packets):
        ...
        descriptors.append(
            f"    {{{packet['frame']}u, {packet['phase']:.2f}f, {prefix}Vertices, "
...
def render_cs(packets: list[dict[str, Any]], report_path: Path) -> str:
    frames = ", ".join(str(row["frame"]) for row in packets)
    phases = ", ".join(f"{row['phase']:.2f}f" for row in packets)
...
def build(capture: Path, report_path: Path, output: Path,
    ...
    packets = collect_packets(capture, report)
    text = render_cs(packets, report_path)
