...
def build(game_root: Path) -> dict[str, Any]:
    ...
    return {
        ...
        "rendererConsumers": {
            ...
            "classicMaterializedBackend": "0x18053fc90 copies the complete 0x1a0 source row into materialized object+0x60, retains the backend manager at +0x200, a registration/resource result at +0x210, and auxiliary resources at +0x220/+0x228. 0x18053f690 performs collection/filter association and retirement, not draw execution. The downstream 0x180540280 path consumes +0x200/+0x210/+0x220/+0x228 and +0x50. Full logical routine 0x180540510..0x18054160a filters source-row flags/IDs/ranges and emits 0x50-byte CPU records; 0x1805438e0..0x180543bd1 partitions/copies their ranges, while generic 0x180555180 emits a 0x18-byte index table stored at object+0x208 with count at +0x210. 0x180541680 allocates 0x190-byte callback rows, initializes +0x10/+0x18 to zero, installs callback 0x180542e10 through the external manager's vtable+0xc28, and that manager must populate row+0x10 before callback execution. 0x180542e10 consumes the generated indices and forwards row+0x10 as argument six to resource/parameter builder 0x1805419d0. Windows x64 entry-stack accounting (including the return address) maps the builder's rbp+0xa08 reload to argument six; its terminal object-vtable+0x210 call therefore targets the same dynamic resource/parameter-provider object. Its concrete external manager implementation, allocation, and vtable remain unresolved. None of these paths statically proves a graphics API draw or submit",
            "classicResourceParameterProvider": {
                "callbackRowBuilder": "0x180541680",
                "callbackRowStride": "0x190",
                "externalManagerRegistration": "0x18054189a calls manager vtable+0xc28 with callback 0x180542e10",
                "providerField": "callback row+0x10; zero at construction and populated before callback execution by the external manager",
                "builderArgument": 6,
