...
def verify(path: Path) -> dict:
    ...
    return {"status":"verified","mesh":EXPECTED,"channels":CHANNELS,"source":SOURCE,"scope":SCOPE,"nativeGate":"unavailable"}
...
def verify_report(report_path: Path, mesh_path: Path | None = None) -> dict:
    ...
    if r.get("source")!=SOURCE or r.get("scope") != SCOPE: raise RuntimeError("durable report scope/source mismatch")
    return actual or r
