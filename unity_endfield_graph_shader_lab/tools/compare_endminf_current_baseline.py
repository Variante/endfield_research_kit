...
def verify() -> dict:
    ...
    for phase,src_frame,name in CASES:
        ...
        unity_occlusion=unity_mask>0
        kernel=np.ones((9,9),np.uint8)
        common=(valid_crop>0) & (cv2.dilate(actor_occlusion.astype(np.uint8),kernel)==0) & (cv2.dilate(unity_occlusion.astype(np.uint8),kernel)==0)
        ...
        paired_src=src[common]; paired_unity=unity[common]; delta=np.abs(paired_src.astype(np.int16)-paired_unity.astype(np.int16))
        rows.append({'phase':phase,'sourceFrame':src_frame,'sourceTimeSeconds':round(src_frame/FPS,6),'unityRender':name,'unityPhaseTimeSeconds':float(token),'phaseMapping':'explicitly unpaired; temporal correspondence unknown','sourceSha256':sha(SOURCE),'actorMatteSha256':sha(ACTOR),'unitySha256':sha(rp),'unityObjectIdMaskSha256':sha(unity_mask_path),'validPixels':int((compare_mask>0).sum()),'backgroundPixels':int(background_mask.sum()),'commonMaskPixels':int(common.sum()),'eccTranslation':score,'translationPixels':translation,'sourceBackgroundMeanBgr':[round(float(v),4) for v in pixels.mean(axis=0)],'sourceBackgroundStdBgr':[round(float(v),4) for v in pixels.std(axis=0)],'pairedBgrMeanSource':[round(float(v),4) for v in paired_src.mean(axis=0)],'pairedBgrMeanUnity':[round(float(v),4) for v in paired_unity.mean(axis=0)],'pairedMaeBgr':round(float(delta.mean()),4),'pairedP95Bgr':round(float(np.percentile(delta,95)),4),'diagnosticDiff':{'path':str(diff_path.relative_to(REPO)).replace('\\','/'),'sha256':sha(diff_path),'description':'4x absolute BGR difference in exact crop; diagnostic only'},'claim':'diagnostic only; unpaired samples are excluded from phase-quality claims and no perceptual equivalence is asserted'})
    return {'schema':'endfield.character-recovery.endminf-current-baseline.v1','status':'sparse_unpaired_baseline','sourceViewport':{'path':str(VIEW.relative_to(REPO)).replace('\\','/'),'contractSha256':sha(VIEW_CONTRACT),'originalSha256':sha(SOURCE),'maskSha256':sha(MASK),'actorMattePath':str(ACTOR.relative_to(REPO)).replace('\\','/'),'actorMatteManifestSha256':sha(ACTOR_MANIFEST),'actorMatteSha256':sha(ACTOR),'actorSourceFrameStart':ACTOR_START,'cropHalfOpen':list(CROP),'sourceFrameStart':SOURCE_START,'fps':FPS,'validityRule':'common mask = source-valid & not dilated source actor & not dilated Unity object-ID actor; dilation=9; no synthesized pixels'},'unityPhaseRenders':{'root':str(RENDER.relative_to(REPO)).replace('\\','/'),'availablePhases':[r[0] for r in CASES],'missingPhases':list(MISSING),'phaseNote':'Current outputs are sparse and explicitly unpaired; start and transition captures are absent.'},'comparisons':rows,'diagnostics':{'backgroundMismatch':'paired common-mask BGR metrics are structural diagnostics only','actorRenderMismatch':'not measured without exact temporal bridge','perceptualEquivalence':False,'diffDirectory':str(DIFF.relative_to(REPO)).replace('\\','/'),'phaseQualityClaims':False}}
def main():
