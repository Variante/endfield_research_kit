(() => {
  ...
  const TEXT = {
    en: {
      ...
      inactiveProducer: "inactive current fallback producer",
  ...
  function runtimeContractHtml() {
    ...
    return `<details class="mp-contract-details" data-weight="strong"><summary>${esc(t("nativeBoundary"))} · ${rows.length} ${esc(t("exchanges"))}${localRows.length ? ` · ${localRows.length} ${esc(t("localOnlyPaths"))}` : ""}${protocolOnlyRows.length ? ` · ${protocolOnlyRows.length} ${esc(protocolLabel("capability"))}` : ""}</summary>
      ...
      ${missionOptionAudit?.finding || missionPropertyAudit?.finding || paramSourceAudit?.finding || managedCarrierCensus?.finding || nestedCarrierCensus?.finding ? `<section class="mp-local-only mp-carrier-audits"><header><strong>${esc(t("carrierAudit"))}</strong><span>${esc(t("noGraphEdges"))}</span></header><div>
        ...
        ${nestedCarrierCensus?.finding ? `<article class="mp-contract-card is-boundary-only"><span>${esc(nestedCarrierCensus.classification || "all_nested_managed_identity_carriers_reviewed")}</span><strong>${esc(t("nestedManagedCandidates"))}: ${Number(nestedCarrierCensus.metadata?.candidateTypes || 0).toLocaleString()}</strong><div class="mp-contract-tags"><b>${esc(t("nestedDependentCandidates"))}: ${Number(nestedCarrierCensus.metadata?.nestedDependentCandidateTypes || 0).toLocaleString()}</b><b>${esc(t("maxTypedDepth"))}: ${Number(nestedCarrierCensus.metadata?.maxCustomTypeDepth || 0).toLocaleString()}</b><b>${esc(t("unreviewedCandidates"))}: ${Number(nestedCarrierCensus.metadata?.unreviewedCandidateTypes || 0).toLocaleString()}</b><b>${esc(t("inactiveProducer"))}</b></div><small><b>${esc(t("protocolFields"))}:</b> <code>m_pendingItemSubmitter ${esc(nestedCarrierCensus.pendingItemSubmitterClosure?.fields?.["DialogManager.m_pendingItemSubmitter"]?.token || "")}@${esc(nestedCarrierCensus.pendingItemSubmitterClosure?.fields?.["DialogManager.m_pendingItemSubmitter"]?.offset || "")}</code> <code>questId ${esc(nestedCarrierCensus.pendingItemSubmitterClosure?.fields?.["InventoryItemSubmitter.questId"]?.token || "")}@${esc(nestedCarrierCensus.pendingItemSubmitterClosure?.fields?.["InventoryItemSubmitter.questId"]?.offset || "")}</code></small><code>${(nestedCarrierCensus.pendingItemSubmitterClosure?.methods || []).map((row) => `${esc(row.symbol || "")}${row.address ? ` @ ${esc(row.address)}` : ""} [${Number(row.directCallerCount || 0).toLocaleString()} callers]`).join(" 路 ")}</code><p>${esc(nestedCarrierCensus.finding)}</p><small>${esc(nestedCarrierCensus.pendingItemSubmitterClosure?.finding || "")}</small><small>${esc(nestedCarrierCensus.boundary || "")}</small></article>` : ""}
      </div></section>` : ""}
