(() => {
  ...
  const TEXT = {
    en: {
      ...
      dialogTreeValidatedFinishEndpoints: "validated prime-reachable finish endpoints",
      dialogTreeRejectedFinishEndpoints: "rejected finish endpoints",
      ...
      dialogTreeCorpusRouteGaps: "full-corpus route gaps",
      dialogTreeUnreferencedDefinitionRoutes: "unreferenced definition options",
      dialogTreeZeroEdgeRoutes: "options on linked zero-edge nodes",
      dialogTreeOutOfBoundsRoutes: "out-of-bounds indexes on partial nodes",
      dialogTreeRoutingClass: "routing class",
      dialogTreeIncomingConnections: "incoming connections",
      dialogTreeRouteTopology: "Physical option-edge routing",
      dialogTreeRouteIndex: "connection index",
      dialogTreeRouteHint: "The original binary sets doNext while showing normal options, then passes NormalOptionData.index to DialogTree.Continue. Missing edges are not terminal choices: unequal counts are valid only when exact indexes resolve; unreferenced definitions, zero-edge nodes, and invalid indexes remain visible and fail closed.",
      ...
      dialogFinishEndpointStat: "endpoint-only dependencies",
      dialogFinishExactCoverageStat: "exact finish consumers covered",
      ...
      teleportCarrierAudit: "generic native carrier audit",
      teleportCarrierSignatureMethods: "signature methods",
      teleportCarrierPaths: "nested carrier paths",
      teleportCarrierCallsites: "direct callsites",
      teleportCarrierZeroInitializers: "zero-initialized direct arguments",
      ...
      dialogConditionalCount: "binary local DialogTree conditionals",
      dialogTreeBranchNode: "DialogTree branch node",
      dialogTreeBranchArm: "condition arm",
      ...
      nativeSerializedBranchMissionContext: "mission serialized Branch context",
      nativeSerializedBranchMissionContextHint: "These rows are exact Story-key intersections projected from the original LevelScript census. Related original files are attached for review; the projection does not prove mission ownership, activation, arm exclusivity, or Story file order.",
      nativeSerializedBranchMissionKeys: "mission Story keys",
      nativeSerializedBranchExternalKeys: "external Story keys",
      ...
      storyBranchShell: "Story-branch evidence shell",
      storyBranchShellHint: "No MissionRuntimeAsset exists for this Story namespace. This page exposes hash-validated authored branch and validation context only; it is not a mission or quest owner and does not establish Story-file order.",
      ...
      storyBranchRelatedFiles: "Original Story-branch files",
    ...
    zh: {
      ...
      dialogTreeValidatedFinishEndpoints: "已验证主入口可达结束端点",
      dialogTreeRejectedFinishEndpoints: "已拒绝结束端点",
      dialogTreeDefaultIndexes: "运行时默认连接索引",
      dialogTreeMismatchNodes: "选项/连接数量不等的节点",
      dialogTreeExtraOptionNodes: "额外选项节点",
      dialogTreeCorpusRouteGaps: "全语料路径缺口",
      dialogTreeUnreferencedDefinitionRoutes: "未被图引用的定义选项",
      dialogTreeZeroEdgeRoutes: "已连接零输出边节点上的选项",
      dialogTreeOutOfBoundsRoutes: "部分连接节点上的越界索引",
      dialogTreeRoutingClass: "路径结构分类",
      dialogTreeIncomingConnections: "输入连接",
      dialogTreeRouteTopology: "选项到物理边的路径",
      dialogTreeRouteIndex: "连接索引",
      dialogTreeRouteHint: "原始二进制在显示普通选项时设置 doNext，随后把 NormalOptionData.index 传给 DialogTree.Continue。缺失输出边不是结局选项：只有精确索引可解析时数量不等才有效；未引用定义、零输出边节点和无效索引都会保留显示并关闭推断。",
      ...
      dialogFinishEndpointStat: "仅结束端点依赖",
      dialogFinishExactCoverageStat: "已覆盖精确完成条件",
      ...
      storyBranchShell: "Story \u5206\u652f\u8bc1\u636e\u5916\u58f3",
      storyBranchShellHint: "\u8be5 Story \u547d\u540d\u7a7a\u95f4\u6ca1\u6709 MissionRuntimeAsset\u3002\u672c\u9875\u4ec5\u5c55\u793a\u5df2\u54c8\u5e0c\u9a8c\u8bc1\u7684\u4f5c\u8005\u5206\u652f\u548c\u9a8c\u8bc1\u4e0a\u4e0b\u6587\uff1b\u4e0d\u662f\u4efb\u52a1\u6216\u8282\u70b9\u5f52\u5c5e\uff0c\u4e5f\u4e0d\u5efa\u7acb Story \u6587\u4ef6\u987a\u5e8f\u3002",
      sourceOrderRelatedFiles: "\u539f\u59cb\u987a\u5e8f\u6587\u4ef6",
      sourceOrderRelatedFilesHint: "\u8fd9\u4e9b\u5df2\u54c8\u5e0c\u9a8c\u8bc1\u7684\u539f\u59cb\u6587\u4ef6\u4ec5\u4e3a\u4e25\u683c\u6e90\u6570\u636e\u987a\u5e8f\u62a5\u544a\u63d0\u4f9b\u5ba1\u8ba1\u53ef\u8ffd\u6eaf\u6027\uff1b\u4e0d\u5efa\u7acb\u4efb\u52a1\u5f52\u5c5e\u3001\u6fc0\u6d3b\u3001\u5206\u652f\u9009\u62e9\u6216\u5b8c\u6574 Story \u6587\u4ef6\u987a\u5e8f\u3002",
      storyBranchRelatedFiles: "\u539f\u59cb Story \u5206\u652f\u6587\u4ef6",
      ...
      teleportCarrierAudit: "\u901a\u7528\u539f\u751f\u8f7d\u4f53\u5ba1\u8ba1",
      teleportCarrierSignatureMethods: "\u7b7e\u540d\u65b9\u6cd5",
      teleportCarrierPaths: "\u5d4c\u5957\u8f7d\u4f53\u8def\u5f84",
      teleportCarrierCallsites: "\u76f4\u63a5\u8c03\u7528\u70b9",
      teleportCarrierZeroInitializers: "\u96f6\u521d\u59cb\u5316\u76f4\u63a5\u53c2\u6570",
      ...
      dialogConditionalCount: "\u4e8c\u8fdb\u5236\u9a8c\u8bc1\u7684\u672c\u5730 DialogTree \u6761\u4ef6\u5206\u652f",
      dialogTreeBranchNode: "DialogTree \u5206\u652f\u8282\u70b9",
      dialogTreeBranchArm: "\u6761\u4ef6\u5206\u652f\u81c2",
      ...
      nativeSerializedBranchMissionContext: "\u4efb\u52a1\u5e8f\u5217\u5316 Branch \u4e0a\u4e0b\u6587",
      nativeSerializedBranchMissionContextHint: "\u8fd9\u4e9b\u884c\u662f\u539f\u59cb LevelScript \u666e\u67e5\u4e2d\u4e0e\u8be5\u4efb\u52a1 Story \u952e\u7cbe\u786e\u4ea4\u96c6\u540e\u7684\u6295\u5f71\u3002\u76f8\u5173\u539f\u59cb\u6587\u4ef6\u5df2\u9644\u52a0\u4f9b\u590d\u6838\uff1b\u8be5\u6295\u5f71\u4e0d\u8bc1\u660e\u4efb\u52a1\u5f52\u5c5e\u3001\u6fc0\u6d3b\u3001\u5206\u652f\u4e92\u65a5\u6027\u6216 Story \u6587\u4ef6\u987a\u5e8f\u3002",
      nativeSerializedBranchMissionKeys: "\u4efb\u52a1 Story \u952e",
      nativeSerializedBranchExternalKeys: "\u5916\u90e8 Story \u952e",
  ...
  function missionBadges(row) {
    ...
    if (row.sourceOrderShell) badges.push(`<span class="mp-list-badge is-evidence">${esc(t("sourceOrderShell"))}</span>`);
    if (row.storyBranchShell) badges.push(`<span class="mp-list-badge is-evidence">${esc(t("storyBranchShell"))}</span>`);
  ...
  function storyConnectionOriginalFilesHtml(row) {
    const files = (row?.relatedOriginalFiles || [])
      .filter((file) => file && file.sourceFile)
      .map((file) => `<small><code>${esc(file.kind || "file")}</code> <code>${esc(file.sourceFile)}</code>${file.sha256 ? ` / SHA-256 <code>${esc(file.sha256)}</code>` : ""}</small>`)
      .join("");
    const validation = row?.relatedOriginalFilesValidation || {};
    const unresolved = (validation.unresolvedSourceReferences || [])
      .filter(Boolean)
  ...
  function unassignedStoryHtml() {
    ...
    const timelineActivationRoutesById = new Map(
      (timelineRuntimeAudit.activationRoutes || [])
        .filter((row) => row?.id)
        .map((row) => [row.id, row]),
    );
    ...
    const timelineRuntimeHtml = (key) => {
      ...
      const seenActivationRoutes = new Set();
      rows.forEach((row) => (row.parentDialogActivationRouteIds || []).forEach((routeId) => {
        const route = timelineActivationRoutesById.get(routeId);
  ...
  function storyOrderHtml() {
    ...
    const relatedOriginalFilesHtml = (row) => (row.relatedOriginalFiles || []).length
      ...
      : "";
    const missionAreaLevelDataContexts = (order.missionAreaLevelDataReceiverContexts || []).map((row) => {
    ...
    const nativeReceiverStoryContexts = (order.nativeReceiverStoryContexts || []).map((row) => {
      const receiver = row.receiverEvidence || {};
      ...
      const teleport = teleportRows.map((item) => {
        const carrier = item.carrierAudit || {};
        const initializerStates = carrier.actionIdInitializerStates || {};
        const carrierLine = carrier.schema
          ? `<small><strong>${esc(t("teleportCarrierAudit"))}:</strong> <code>${esc(carrier.validationStatus || "?")}</code> / ${Number(carrier.signatureMethodCount || 0).toLocaleString()} ${esc(t("teleportCarrierSignatureMethods"))} / ${Number(carrier.containerPathCount || 0).toLocaleString()} ${esc(t("teleportCarrierPaths"))} / ${Number(carrier.directCallsiteCount || 0).toLocaleString()} ${esc(t("teleportCarrierCallsites"))} / ${Number(initializerStates.zero || 0).toLocaleString()} ${esc(t("teleportCarrierZeroInitializers"))}<br><code>${esc(carrier.reportJson || "")}</code></small><small>${esc(carrier.producerFinding || "")}</small>`
          : "";
        return `<small><strong>${esc(t("teleportFinishCorrelation"))}:</strong> <code>#${esc(item.listenerHeaderLocalId ?? "?")}</code> <code>${esc(item.actionIdFilter || "?")}</code> <code>${esc(item.classification || "?")}</code> / ${Number(item.externalSerializedOccurrenceCount || 0).toLocaleString()} ${esc(t("teleportFinishExternalOccurrences"))} / ${Number(item.serializedActionCandidateCount || 0).toLocaleString()} ${esc(t("teleportFinishActionCandidates"))}${item.classification === "runtime_only_no_serialized_levelscript_producer" ? ` / <b>${esc(t("teleportFinishRuntimeOnly"))}</b>` : ""}</small><small>${Number(item.corpusFileCount || 0).toLocaleString()} ${esc(t("teleportFinishCorpusFiles"))} / ${Number(item.corpusListenerCount || 0).toLocaleString()} ${esc(t("teleportFinishCorpusListeners"))} / ${Number(item.corpusDistinctFilterCount || 0).toLocaleString()} ${esc(t("teleportFinishCorpusFilters"))}</small>${carrierLine}<small>${esc(item.evidenceBoundary || "")}</small>`;
      }).join("");
      const files = relatedOriginalFilesHtml(row);
    ...
    const serializedBranchStoryKeys = (row) => {
      const keys = new Set();
      const visit = (value) => {
        if (Array.isArray(value)) {
          value.forEach(visit);
        } else if (value && typeof value === "object") {
          ["playbackStoryKeys", "storyKeys"].forEach((field) => {
            const values = value[field];
            if (Array.isArray(values)) values.forEach((key) => key && keys.add(String(key)));
            else if (values) keys.add(String(values));
          });
    ...
    const attachedSerializedBranchContexts = (branches.nativeSerializedBranchContexts || [])
      .filter((row) => row && row.relation === "serialized_branch_story_context");
    const serializedBranchInventoryRows = attachedSerializedBranchContexts.length
      ? attachedSerializedBranchContexts
      : (serializedBranchInventory?.rows || [])
        .filter((row) => [...serializedBranchStoryKeys(row)].some((key) => missionStoryKeys.has(key)));
    const serializedBranchMissionContextHtml = attachedSerializedBranchContexts.length
      ? `<p><strong>${esc(t("nativeSerializedBranchMissionContext"))}: ${attachedSerializedBranchContexts.length.toLocaleString()}</strong> <span>${esc(t("nativeSerializedBranchMissionKeys"))}: ${[...new Set(attachedSerializedBranchContexts.flatMap((row) => row.missionStoryKeys || []))].length.toLocaleString()}</span></p><p>${esc(t("nativeSerializedBranchMissionContextHint"))}</p>`
      : "";
    const serializedBranchInventoryArmHtml = (row) => (row.arms || []).map((arm) => {
      ...
      const nestedControlsHtml = (arm.nestedControls || []).map((control) => {
        ...
        const nestedPredicateGap = control.playbackPredicateStatus === "unresolved_playback_predicate"
          ...
          : "";
        const nestedSchemaGap = control.armSchemaStatus === "unavailable_fail_closed"
          ? `<b>${esc(t("nativeSerializedNestedSchemaGaps"))}</b>`
          : "";
        return `<details><summary><b><code>#${esc(control.localId ?? "?")} ${esc(control.actionName || control.recordClass || "?")}</code></b><span>${Number(control.serializedArmCount || 0).toLocaleString()} ${esc(t("nativeSerializedNestedArms"))}</span>${nestedPlaybackSummary}${nestedBranchingSummary}${nestedPredicateGap}${nestedSchemaGap}</summary>${detail}${predicate}${nestedArms}</details>`;
      }).join("");
    ...
    const dialogTreeBranchNodes = (branches.dialogTreeBranchNodes || []).map((row) => {
      const nativeProof = (row.nativeConsumers || []).map((consumer) => `<code>${esc(consumer.method || "?")} ${esc(consumer.token || "")} @ ${esc(consumer.address || "?")}</code>`).join(" ");
      const arms = (row.arms || []).map((arm) => {
        const condition = arm.condition && Object.keys(arm.condition).length
          ? `<code>${esc(JSON.stringify(arm.condition))}</code>`
          : `<span>${esc(t("nativePredicateOpaque"))}</span>`;
        const target = arm.targetNodeId
          ? `<code>#${esc(arm.targetNodeId)}</code>${arm.targetNodeType ? ` <small>${esc(arm.targetNodeType)}</small>` : ""}`
          : "";
        const defaultArm = arm.defaultArm ? `<b>${esc(t("dialogTreeBranchSelection"))}</b>` : "";
        return `<div><b>${esc(t("dialogTreeBranchArm"))} ${esc(arm.conditionOrdinal ?? "?")}</b><span>${condition}</span><code>connection ${esc(arm.connectionOrdinal ?? "?")} / raw ${esc(arm.connectionIndex ?? "?")}</code><span>&rarr; ${target}</span>${defaultArm ? `<small>${defaultArm}</small>` : ""}</div>`;
      }).join("");
      const originals = (row.relatedOriginalFiles || []).map((related) => `<small><code>${esc(related.kind || "file")}</code> <code>${esc(related.sourceFile || "")}</code>${related.sha256 ? ` / SHA-256 <code>${esc(related.sha256)}</code>` : ""}</small>`).join("");
  ...
  function renderMissionSummary() {
    ...
    const sourceOrderShell = mission.sourceOrderShell === true;
    const storyBranchShell = mission.storyBranchShell === true;
    const aggregateShell = mission.storyAggregateShell === true;
    const shellLabel = aggregateShell ? t("storyAggregateShell") : sourceOrderShell ? t("sourceOrderShell") : storyBranchShell ? t("storyBranchShell") : t("offlineMissionShell");
    const shellHint = aggregateShell ? t("storyAggregateShellHint") : sourceOrderShell ? t("sourceOrderShellHint") : storyBranchShell ? t("storyBranchShellHint") : t("offlineMissionShellHint");
    ...
    const storyBranchRelatedFiles = !aggregateShell
      ? (mission.storyBranchRelatedOriginalFiles || [])
      : [];
    const storyBranchOriginals = storyBranchRelatedFiles.map((related) => `<small><code>${esc(related.kind || "file")}</code> <code>${esc(related.sourceFile || "")}</code>${related.sha256 ? ` / SHA-256 <code>${esc(related.sha256)}</code>` : ""} / <code>${esc(related.relationship || "")}</code></small>`).join("");
    const hasSourceOrderFiles = Boolean(sourceOrderOriginals);
    const hasStoryBranchFiles = Boolean(storyBranchOriginals);
    const metrics = [
    ...
    target.innerHTML = `<div class="mp-summary-head">
      ...
      <div class="mp-case${caseStudy || offlineShell || sourceOrderShell || storyBranchShell || aggregateShell || hasSourceOrderFiles || hasStoryBranchFiles ? " has-case" : ""}">
        <span class="mp-case-icon" aria-hidden="true">${caseStudy ? "◎" : "○"}</span>
         <div><strong>${esc(offlineShell || sourceOrderShell || storyBranchShell || aggregateShell ? shellLabel : hasSourceOrderFiles ? t("sourceOrderRelatedFiles") : hasStoryBranchFiles ? t("storyBranchRelatedFiles") : (caseStudy?.title || t("evidence")))}</strong><p>${esc(offlineShell || sourceOrderShell || storyBranchShell || aggregateShell ? shellHint : hasSourceOrderFiles ? t("sourceOrderRelatedFilesHint") : hasStoryBranchFiles ? t("storyBranchRelatedFilesHint") : (caseStudy?.summary || t("noCase")))}</p>${aggregateVariants ? `<div><b>${esc(t("storyAggregateVariants"))}:</b> ${aggregateVariants}</div>` : ""}${aggregateOriginals ? `<details><summary>${esc(t("storyAggregateOriginals"))} <span>${mission.relatedOriginalFiles.length}</span></summary>${aggregateOriginals}</details>` : ""}${sourceOrderOriginals ? `<details><summary>${esc(t("sourceOrderRelatedFiles"))} <span>${sourceOrderRelatedFiles.length}</span></summary><p>${esc(mission.sourceOrderRelatedFilesBoundary || t("sourceOrderRelatedFilesHint"))}</p>${sourceOrderOriginals}</details>` : ""}${storyBranchOriginals ? `<details><summary>${esc(t("storyBranchRelatedFiles"))} <span>${storyBranchRelatedFiles.length}</span></summary><p>${esc(mission.storyBranchRelatedFilesBoundary || t("storyBranchRelatedFilesHint"))}</p>${storyBranchOriginals}</details>` : ""}${caseStudy ? `<span class="mp-confidence">${esc(t("confidence"))}: ${esc(caseStudy.confidence)}</span>` : ""}</div>
      </div>
      ${offlineShell || sourceOrderShell || storyBranchShell || aggregateShell ? "" : `<div class="mp-mission-handshake">
  ...
  function runtimeContractHtml() {
    ...
    const identityCarrierAudit = contract.identityCarrierBoundaries || {};
    const identityCarrierBoundaries = identityCarrierAudit.status === "validated"
      ? (identityCarrierAudit.boundaries || [])
      : [];
    ...
    return `<details class="mp-contract-details" data-weight="strong"><summary>${esc(t("nativeBoundary"))} · ${rows.length} ${esc(t("exchanges"))}${localRows.length ? ` · ${localRows.length} ${esc(t("localOnlyPaths"))}` : ""}${protocolOnlyRows.length ? ` · ${protocolOnlyRows.length} ${esc(protocolLabel("capability"))}` : ""}</summary>
      ...
      ${identityCarrierBoundaries.length ? `<section class="mp-local-only mp-carrier-audits"><header><strong>${esc(t("carrierAudit"))}</strong><span>${esc(t("noGraphEdges"))}</span></header><div>
        ${identityCarrierBoundaries.map((boundary) => `<article class="mp-contract-card is-boundary-only">
          <span>${esc(boundary.classification || "reviewed_negative_boundary")}</span>
          <strong>${esc(boundary.subject || boundary.id || "")}</strong>
          <div class="mp-contract-tags"><b>${esc(t("boundaryOnly"))}</b><b>${esc(t("noGraphEdges"))}</b></div>
          <p>${esc(boundary.finding || "")}</p>
          <small>${esc(boundary.boundary || "")}</small>
        </article>`).join("")}
        <small><b>${esc(t("relatedOriginalFile"))}:</b> <code>${esc(identityCarrierAudit.sourceFile || "")}</code> / SHA-256 <code>${esc(identityCarrierAudit.sourceSha256 || "")}</code></small>
      </div></section>` : ""}
      ...
      ${receiverNodes.length ? `<section class="mp-missionless-runtime mp-native-receiver-runtime">
        ...
        <div class="mp-missionless-runtime-grid">${receiverNodes.map((row) => {
          ...
          const missionAreaShell = activation.missionAreaLevelDataShellContext || {};
          ...
          return `<article>
            ...
            ${activation.activationClass ? `<div class="mp-runtime-associations">
              ...
              ${missionAreaShell.status ? `<div class="is-boundary"><span>${esc(t("missionAreaLevelDataReceiverScope"))}</span><i aria-hidden="true">&harr;</i><code>${esc((missionAreaShell.hostMissionIds || []).join(", ") || "?")}</code><b>${esc(missionAreaShell.status)}</b><small>${esc(t("missionAreaLevelDataReceiverContextsHint"))}</small></div>` : ""}
  ...
  function objectiveHtml(objective, questId) {
    ...
    const dialogFinishEndpointDependencies = (objective.dialogFinishEndpointDependencies || []).map((row) => {
      const producerEvidence = row.producerEvidence || [];
      const finishNodes = producerEvidence.map((producer) => `<code>${esc(producer.finishNodeId || "?")}</code>`).join(" ");
      const valueSources = [...new Set(producerEvidence.map((producer) => producer.finishIdSource || "unknown"))];
      const defaultBadge = valueSources.includes("runtime_default") ? `<span>${esc(t("dialogFinishRuntimeDefault"))}</span>` : `<span>${esc(t("dialogFinishExplicitValue"))}</span>`;
      const endpointPaths = producerEvidence.map((producer) => {
        const path = (producer.nodePath || []).join(" → ") || "?";
        const predecessors = (producer.predecessorNodeTypes || []).join(" + ") || "?";
        return `<small><strong>${esc(t("dialogFinishPrimePath"))}:</strong> <code>${esc(path)}</code> · ${esc(t("predecessor"))}: <code>${esc(predecessors)}</code></small>`;
      }).join("");
      const files = (row.relatedOriginalFiles || []).map((file) => `<small><code>${esc(file.kind || "file")}</code> <code>${esc(file.sourceFile || "")}</code>${file.sha256 ? ` / SHA-256 <code>${esc(file.sha256)}</code>` : ""}</small>`).join("");
      return `<details open class="mp-quest-task-dependency mp-dialog-finish-branch-dependency">
        <summary><b>${esc(t("dialogFinishEndpointDependencies"))}</b> <code>${esc(row.dialogId || "?")} / ${esc(t("finish"))} ${esc(row.finishId ?? "?")}</code><i>&rarr;</i><code>${esc(row.questId || questId)} / #${esc(row.objectiveIndex ?? "?")}</code> ${defaultBadge}</summary>
        <p><strong>${esc(t("dialogFinishProducer"))}:</strong> ${finishNodes || `<code>?</code>`}</p>
        ${endpointPaths}
        <small><strong>${esc(t("dialogFinishConsumer"))}:</strong> <code>${esc(row.conditionId || "?")}</code> / <code>CheckTalkOptionFinish</code></small>
        <small>${esc(row.evidenceBoundary || t("dialogFinishEndpointDependencyHint"))}</small>
        ${files ? `<details><summary>${esc(t("relatedOriginalFile"))} <span>${(row.relatedOriginalFiles || []).length}</span></summary>${files}</details>` : ""}
      </details>`;
    }).join("");
  ...
  function dialogTreeDefinitionsHtml(node) {
    ...
    const rows = definitions.map((row) => {
      ...
      const finishEndpointRecovery = row.finishEndpointRecovery || {};
      const finishEndpointCounts = finishEndpointRecovery.counts || {};
      const facts = [
        ...
        `${Number(finishEndpointCounts.validatedFinishEndpoints || 0)} ${t("dialogTreeValidatedFinishEndpoints")}`,
        `${Number(finishEndpointCounts.rejectedFinishEndpoints || 0)} ${t("dialogTreeRejectedFinishEndpoints")}`,
      ];
      ...
      const routeTopology = routeNodes.length ? `<details class="mp-dialog-tree-route-topology"><summary><b>${esc(t("dialogTreeRouteTopology"))}</b> · ${Number(routeCounts.connectionCountMismatchNodes || 0).toLocaleString()} ${esc(t("dialogTreeMismatchNodes"))} · ${Number(routeCounts.extraOptionNodes || 0).toLocaleString()} ${esc(t("dialogTreeExtraOptionNodes"))}</summary><small>${esc(t("dialogTreeRouteHint"))}</small>${routeNodes.map((routeNode) => {
        const routes = (routeNode.routes || []).map((route) => {
          const target = route.status === "validated"
            ...
            : ` · <strong>${esc(route.status || "rejected")}</strong> · <code>${esc(route.failureClass || "unclassified_route_failure")}</code> · ${esc(route.issue?.gate || "validation_failed")}`;
        ...
        return `<article class="mp-action"><small><code>${esc(routeNode.nodeId || "?")}</code> · ${Number(routeNode.normalOptionCount || 0)} ${esc(t("dialogTreeNormalOptions"))} / ${Number(routeNode.outgoingConnectionCount || 0)} ${esc(t("dialogTreeConnections"))} · ${esc(t("dialogTreeIncomingConnections"))} ${Number(routeNode.incomingConnectionCount || 0)} · ${esc(t("dialogTreeRoutingClass"))} <code>${esc(routeNode.routingClass || "unclassified")}</code></small><ul>${routes}</ul></article>`;
      }).join("")}</details>` : "";
      const finishEndpoints = finishEndpointRecovery.endpoints || [];
      const finishEndpointTopology = finishEndpoints.length ? `<details class="mp-dialog-tree-route-topology"><summary><b>${esc(t("dialogFinishEndpointDependencies"))}</b> · ${Number(finishEndpointCounts.validatedFinishEndpoints || 0).toLocaleString()} ${esc(t("dialogTreeValidatedFinishEndpoints"))} / ${Number(finishEndpointCounts.rejectedFinishEndpoints || 0).toLocaleString()} ${esc(t("dialogTreeRejectedFinishEndpoints"))}</summary><small>${esc(t("dialogFinishEndpointDependencyHint"))}</small><ul>${finishEndpoints.map((endpoint) => {
        const finishValue = endpoint.finishId == null ? "?" : endpoint.finishId;
        const path = (endpoint.nodePath || []).join(" → ");
        const predecessors = (endpoint.predecessorNodeTypes || []).join(" + ");
        const failure = endpoint.status === "validated" ? "" : ` · <strong>${esc(endpoint.status || "rejected")}</strong> · <code>${esc(endpoint.failureClass || "unclassified_finish_endpoint_failure")}</code> · ${esc(endpoint.issue?.gate || "validation_failed")}`;
        return `<li><code>${esc(endpoint.nodeId || `#${endpoint.nodeOrdinal ?? "?"}`)}</code> · ${esc(t("finish"))} <code>${esc(finishValue)}</code> (${esc(endpoint.finishIdSource || "?")})${path ? ` · ${esc(t("dialogFinishPrimePath"))} <code>${esc(path)}</code>` : ""}${predecessors ? ` · ${esc(t("predecessor"))} <code>${esc(predecessors)}</code>` : ""}${failure}</li>`;
      }).join("")}</ul></details>` : "";
