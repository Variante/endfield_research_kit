(() => {
  ...
  const TEXT = {
    en: {
      ...
      binaryClientActiveRequestBoundary: "The original LevelData type selects this generic runtime branch. It proves where the client produces active=true, but not who supplied Enabled, whether the spatial check passed in a particular run, which server branch accepted Active, mission ownership, event firing, or Story order.",
    ...
    zh: {
      ...
      binaryClientActiveRequestBoundary: "原始 LevelData 类型选择这条通用运行时分支。它证明客户端在何处发出 active=true，但不证明谁提供 Enabled、某次运行是否通过空间检查、服务端选择了哪条分支、任务归属、事件是否发生或 Story 顺序。",
