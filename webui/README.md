...
- `Data`: local-only browser for `export_full/structured/StreamingAssets/Data`, with lazy-loaded group shards, foldable JSON prefix groups, parsed text JSON previews, MemoryPack binary `.json` field summaries for known families, and binary header previews for other payloads. It excludes PCK audio packages and exported videos; the Assets tab owns video browsing and playback.
