...
- `Gameplay`: curated weapon and character records from structured game tables, generated by `python scripts/build_gameplay_data.py`. Weapon rows use `ItemTable.name` for the localized display/file name, follow `WeaponBasicTable.weaponSkillList` into `SkillPatchTable`, and show localized effect text, blackboard values, upgrade checkpoints, stat checkpoints, breakthrough costs, and talent bound templates. Character rows use `CharGrowthTable.skillGroupMap` as the displayed skill source and show linked `SkillPatchTable` action/scaling data, skill level-up costs, grouped break/equipment/attribute/talent nodes, profession, element, default weapon links, level checkpoints, capped character stat checkpoints, break-stage caps, breakthrough costs, and potential unlocks.
