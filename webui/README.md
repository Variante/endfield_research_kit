...
- `Gameplay`: curated weapon and character records from structured game tables, generated by `python scripts/build_gameplay_data.py`. Weapon rows use `ItemTable.name` for the localized display/file name, follow `WeaponBasicTable.weaponSkillList` into `SkillPatchTable`, and display localized effect text plus skill blackboard values; character rows use `CharGrowthTable.skillGroupMap` as the displayed skill source and show linked `SkillPatchTable` action/scaling data, skill level-up costs, talents, profession, element, and default weapon links.
