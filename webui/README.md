...
- `src/features/game_data/`: local decoded StreamingAssets/Persistent Data/Json browser backed by `webui/data/game_data/` indexes.
- `src/features/decoded/`: local AnimeStudio decoded JSON browser backed by `webui/data/decoded/` indexes.
...
- `Gameplay`: curated weapon and character records from structured game tables, generated by `python scripts/build_gameplay_data.py`. Weapon rows use `ItemTable.name` for the localized display/file name, follow `WeaponBasicTable.weaponSkillList` into `SkillPatchTable`, and show localized effect text, blackboard values, upgrade checkpoints, breakthrough costs, and talent bound templates. Character rows use `CharGrowthTable.skillGroupMap` as the displayed skill source and show linked `SkillPatchTable` action/scaling data, skill level-up costs, grouped break/equipment/attribute/talent nodes, profession, element, default weapon links, level checkpoints, break-stage caps, breakthrough costs, and potential unlocks.
...
- `Data`: local-only browser for final decoded config files under `export_full/structured/StreamingAssets/Data/Json` and `export_full/structured/Persistent/Data/Json`, with lazy-loaded group shards, JSON groups folded by clear directory structure before filename prefixes, parsed text JSON previews, and MemoryPack binary `.json` field summaries for known families. Raw `.ab` bundles, packed audio, video/media, streaming, irradiance, and extend-data payloads are intentionally excluded from this page.
- `Decoded`: local-only browser for AnimeStudio JSON outputs under `export_full/recovered/AnimeStudio-cli/<source>/json_by_type/`, generated by `python scripts/build_decoded_index.py`. The default index covers MonoBehaviour, groups entries by semantic domain and schema signal, splits only oversized semantic/schema groups into balanced lazy-loaded parts, adds lightweight semantic meaning/tags from names, classes, layouts, field sets, and decode markers, shows `$animestudio` metadata, managed-reference classes/layouts, top-level fields, schema/field-set IDs, and keeps raw JSON previews behind an opt-in switch without copying decoded files into `webui/data/`.
...
- `data/lang/<code>/gameplay/index.json`: curated Gameplay tab payload generated from structured tables such as `WeaponBasicTable`, `CharacterTable`, `CharGrowthTable`, and `SkillPatchTable`.
- `data/game_data/index.json` and `data/game_data/groups/*.json`: ignored, local-only Data tab indexes generated by `scripts/build_data_index.py`. They summarize final decoded config files under both `StreamingAssets/Data/Json` and `Persistent/Data/Json` and do not copy raw Data files or package/media containers.
- `data/decoded/index.json` and `data/decoded/groups/*.json`: ignored, local-only Decoded tab indexes generated by `scripts/build_decoded_index.py`. They summarize AnimeStudio decoded JSON files and point raw links back to `export_full/recovered/AnimeStudio-cli/...`.
