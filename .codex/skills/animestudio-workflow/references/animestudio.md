...
| Streaming | `scripts/game_data/streaming.py`, `scripts/game_data/streaming_corpus.py` | `scripts/tests/test_streaming.py`, `scripts/tests/test_streaming_corpus.py` (all envelopes/roots, exact anonymous Info graphs, field-2 row slot-span partition plus terminal field-5 width-4 vectors through EOF, field3/4/5 first-level, and field6/7 paired-group subgraphs; slot padding/value layouts, terminal values, deeper parallel field-5 children, and semantics remain opaque) |
