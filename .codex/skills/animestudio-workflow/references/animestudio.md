...
| IFixPatchOut | `scripts/game_data/ifix_patch.py`, `ifix_vm_instruction_native.py`, `ifix_vm_operands_native.py`, and their selected-build contracts | `scripts/tests/test_ifix_patch.py`, `test_ifix_vm_instruction_native.py`, `test_ifix_vm_operands_native.py` (exact spans and opcodes; selected-build call, branch, string, field, frame-slot, and return operand readings; execution open) |
...
| DynamicStreaming | `scripts/game_data/dynamic_streaming.py`, `dynamic_main_native.py`, `dynamic_stream_area_corpus.py`, `dynamic_stream_area_native.py`, and the selected-build main/area contracts | `scripts/tests/test_dynamic_main_native.py`, `test_dynamic_streaming.py` (field-specific main vector widths, area contiguous tail, named index partitions, point containment, and fail-closed native gates) |
