...
| Streaming | `scripts/game_data/streaming.py`, `scripts/game_data/streaming_corpus.py`, `scripts/game_data/streaming_native.py` | `scripts/tests/test_streaming.py`, `scripts/tests/test_streaming_corpus.py`, `scripts/tests/test_streaming_native.py` (all envelopes/roots, exact anonymous Info graphs, field-2 row slot partition plus terminal field-5 width-4 vectors through EOF, selected-build native-gated fields 0--4 representations and current filename-token relations, field3/4/5 first-level, and field6/7 paired-group subgraphs; raw native carrier/final cursor, field names, terminal values, deeper parallel field-5 children, and semantics remain unresolved) |
