...
source-link evidence, rebuilds CN story/reference data, refreshes Story order,
and finishes by linking decoded CN audio by default.
...
full-list file that is also regenerated by the story-order builder.
...
- The builder writes this file by default from recovered game-data order.
...
- Set `missions.<mission>.locked` to `true` to freeze that mission order;
  build_story and OCR recovery must preserve it.
