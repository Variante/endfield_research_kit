...
structured tables, VFS assets, or `GameAssembly.dll` alone. In the current
Endfield install it is an original Unity IL2CPP runtime metadata file under:

```text
D:\Program Files\Endfield Game\Endfield_Data\il2cpp_data\Metadata\global-metadata.dat
```

Treat it as installed game data, not as a file generated by this repo.
...
If a future installed game no longer has a metadata file, use a clean copy from
a previous cache or patch package and pass it via `--metadata`. If no valid
metadata file exists, continue with `export.bat`; only the metadata drift
