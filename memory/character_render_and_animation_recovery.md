...
object offset `0x6c8`. The selected Gacha owner is now source-closed:
`Env_gachaRoom_01` selects active Manual zero-EV exposure, target `1`, and
`0.6/0.6` scaled-`deltaTime` adaptation. Its serialized-disabled Unity
`ExternalCamera` is sampled by `CinemachineExternalCamera` for transform/lens
state; `CinemachineBrain` applies that state to `CameraManager`'s separately
created persistent physical main Camera. Gacha therefore retains that Camera's
per-camera `HGCamera` history. The exact recurrence is
`E[n+1]=1+(E[n]-1)*(1-clamp(0.6*deltaTime[n],0,1))`; the remaining boundary is
only the carry-in `E[0]` and discrete scaled frame-delta sequence, not the
owner, target, formula, or physical-camera route. The producer verifier/report is
...
The Gacha owner/camera-route verifier and report are under
`scratch/reverse_engineering/gacharoom_exposure/`; the report SHA-256 is
