...
2026-07-02 domain depot graph progress: source graph ingestion now promotes authored domain-depot tables directly, including depot definitions, buyers, delivery targets, nested upgrade levels, pack-value ranges, deliver item/pack types, package-integrity reduce types, depot constants, and `DomainPoiTable` feature categories. A fast CN temp build to `tmp/domain_depot_source_graph.sqlite` verified 5 depots, 55 buyers, 22 delivery targets, 7 recycle targets, 19 depot upgrade levels, 45 pack-value rows, 3 deliver item types, 3 deliver pack types, 3 integrity reduce types, 8 constants, 8 domain POI types, 1 depot feature node, 22 target-to-dialog edges, 55 buyer-to-depot shared-level edges, and 22 target-to-depot shared-level edges. Smoke checks passed for `domain_1_lv005_depot_1`, `buyer_map01_lv005_01`, `deliver_target_map01_lv005_01`, upgrade level `domain_1_lv005_depot_1:2`, and POI type `3`. This models authored depot structure and links existing target-dialog nodes; buyer/target depot edges are explicitly marked as shared-level/refLevelId inference, not direct buyer-target matching or live delivery state.

2026-07-02 factory logistics/capability graph progress: source graph ingestion now promotes selected factory logistics and capability tables directly, including machine craft-group recipe membership, ingredient tags, machine/resource capability indexes, item-to-logistics mappings, logistics unit configs, panel-store goods/actions/conditions, building panel locks, blueprint tags/types, system blueprint metadata, factory regions, and blackbox-to-region requirements. A fast CN temp build to `tmp/factory_logistics_source_graph.sqlite` verified 14 ingredient tags, 16 logistics units, 42 panel-store goods, 11 panel locks, 14 panel-lock rules, 20 blueprint tags, 3 blueprint tag types, 45 system blueprints, 57 blackbox nodes, 257 craft-group recipe edges, 14 machine-tag edges, 229 resource item-to-machine edges, 241 recipe item-to-ingredient-tag edges, 20 item-to-logistics-unit edges, 5 panel-condition tech refs, 27 lock quest edges, 5 lock radio edges, 57 blackbox-region edges, and 69 region-level edges. Smoke checks passed for `group_component_normal`, `tag_component`, `grid_belt_01`, `map01_lv001_bus_1`, blueprint tag `101`, `furnance_1`, `blackbox_assebling_1`, and `factory_region_bingganbox`. This models authored capability/index metadata and UI unlock/store wiring; it does not execute factory logistics, simulate recipe throughput, or prove live building placement/state.

2026-07-02 reward/currency catalog graph progress: source graph ingestion now promotes compact reward and currency catalog tables directly, including gift items and preference tags, check-in activities/stages, daily activation rewards, money config/record/exchange rows, important item show categories, and important reward item descriptions. A fast CN temp build to `tmp/reward_catalog_source_graph.sqlite` verified 14 gift items, 6 gift preference tags, 14 check-in activities, 114 check-in stages, 5 daily activation reward rows, 5 money configs, 2 money exchanges, 1 important-item show group, 4 important-item show entries, 12 important reward items, 28 gift hobby-tag edges, 14 gift preference edges, 114 check-in stage reward edges, 1 featured character edge, 1 featured weapon edge, 5 activation reward edges, 4 money exchange source/target item edges, and 12 important reward description text links. Smoke checks passed for `item_gift_jinlong_1`, `activity_checkin_1:3`, `item_originium_to_item_diamond`, activation reward `1`, and `item_activity_formula_xiranite_hulu`. This models authored reward/catalog/currency evidence; it does not prove live account check-in state, inventory ownership, server-side reward claiming, or runtime currency transaction execution.

2026-07-02 activity/adventure catalog graph progress: source graph ingestion now promotes authored adventure book, activity entrance, high-difficulty, PS activity, submit-food, and benefits showcase rows. A fast CN temp build to `tmp/activity_catalog_source_graph.sqlite` verified 85 adventure book tasks, 13 adventure book stages, 1 activity entrance group, 2 activity entrance series, 45 catalog game refs, 4 high-difficulty series, 3 PS activities, 11 PS activity tasks, 14 submit-food stages, 14 submit items, 8 activity benefits, 85 adventure task reward edges, 86 adventure task condition edges, 72 adventure stage task edges, 15 entrance-series game edges, 30 high-difficulty game edges, 22 PS task mission edges, 14 submit-food recipe edges, and 19 benefit reward/showcase refs. Smoke checks passed for `adventure_book_task:ab_01_01`, `activity_game_entrance_series:activity_puzzle_v1d0`, and `activity_benefit:checkIn`. This models authored activity catalog and navigation evidence; it does not prove live event availability, runtime unlock evaluation, account progress, or server-side activity state.

2026-07-02 Week Raid graph progress: source graph ingestion now promotes the authored Week Raid mode catalog from `WeekRaidTable`, `WeekRaidTech*`, `WeekRaidDelegateTable`, `WeekRaidItem*`, `WeekraidItemDomainTable`, `WeekRaidRefreshTable`, and `WeekRaidBattlePassTable`. A fast CN temp build to `tmp/week_raid_source_graph.sqlite` verified 2 Week Raid games, 6 item-domain labels, 7 tech types, 9 buff tech types, 31 tech rows, 66 delegate missions, 9 refresh levels, 20 battle-pass tiers, 2 currency edges, 1 unlock-random-stage mission edge, 31 tech item edges, 66 delegate reward edges, 68 item conversion/reverse edges, 68 item-domain edges, 9 refresh-level edges, 20 tier reward edges, 44 normalized format-text edges, and no camel-case `formatText` edge kinds. Smoke checks passed for `week_raid_game:dung01_wrdg001`, `week_raid_tech:t_1_1`, `week_raid_delegate:db01m2d1`, and `week_raid_battlepass_tier:1`. This models authored Week Raid configuration, currency, tech, mission, reward, and display evidence; it does not prove runtime danger/score formulas, live weekly rotation, account progress, or server-side refresh state.

2026-07-02 factory utility numeric graph progress: source graph ingestion now promotes authored factory utility profiles from `FactoryPowerStationTable`, `FactoryHubTable`, `FactoryPowerPoleTable`, factory power/productivity data types, miner/fuel/battery tables, `LiquidTable`, fluid machine tables, and sewage treatment tables. A fast CN temp build to `tmp/factory_utility_source_graph.sqlite` verified 1 power station, 2 hub power profiles, 4 power poles, 5 power data types, 5 productivity data types, 4 miners, 6 fuel profiles, 5 battery profiles, 11 liquids, 17 fluid-machine refs, 4 sewage levels, 5 unique sewage action params, 10 miner output item edges, 4 miner consume item edges, 6 fuel item edges, 5 battery item edges, 67 empty bottle refs, 67 full bottle refs, 67 empty-to-full bottle pair edges, 67 liquid-to-filled-bottle pair edges, 24 fluid-machine accepted-liquid edges, 1 fluid product liquid edge, 4 sewage plant level edges, 8 sewage action param edges, 4 action level refs, and 4 action machine refs. Smoke checks passed for `factory_miner:miner_2`, `factory_liquid:item_liquid_acid`, `factory_fluid_machine:pump_2`, `sewage_treat_plant:liquidcleanfactory_005_1`, and `factory_sewage_level:liquidcleanfactory_005_1:4`. This models authored utility numerics and references; it does not prove runtime throughput, placement behavior, power simulation, liquid transfer simulation, or account/world upgrade state.

2026-07-02 Tower Defense graph progress: source graph ingestion now promotes authored settlement defense tables from `TowerDefenseTable`, `TowerDefenseGroupTable`, and `TowerDefenseMapTable`. A fast CN temp build to `tmp/tower_defense_source_graph.sqlite` verified 22 tower defense stages, 11 groups, 3 settlement maps, 11 unique spawners, 22 stage definitions, 22 group-to-stage edges, 22 stage-to-settlement-map edges, 22 stage scene-level edges, 22 stage reward edges, 96 per-stage enemy refs, 40 deduped enemy-template refs, 82 recommended battle-building item refs, 22 spawner refs, and 11 group name text links. Smoke checks passed for `tower_defense_stage:stm_hongs_1_1_normal`, `tower_defense_group:stm_hongs_1_1`, and `tower_defense_settlement_map:stm_hongs_1`. This models authored settlement defense configuration, enemy waves, rewards, map/detail images, recommended buildings, scene levels, and spawner ids; it does not prove runtime wave timing, settlement combat behavior, placement validation, or account/world defense progress.

2026-07-02 Snapshot/Kite Station photo-chain graph progress: source graph ingestion now promotes authored photo-taking and Kite Station tables from `SnapshotIdentify*`, `SnapshotFilterTable`, `SnapshotStickerTable`, `ActivitySnapshotChallenge*`, `IdentifyGroupId2ActivitySnapshotStageTable`, and `KiteStation*`. A temp validation build to `tmp/photo_chain_source_graph.sqlite` verified 75 defined snapshot identify groups plus 5 Kite bridge-only group refs, 123 snapshot identify ids, 8 identify types, 11 filters, 17 stickers, 3 activity snapshot mains, 17 activity snapshot stages, 17 activity snapshot challenges, 2 Kite Stations, 7 Kite Station levels, 28 entrust tasks, 9 milestone reward nodes, 136 group-to-identify refs, 17 group-to-activity-stage refs, 17 challenge mission refs, 17 stage level refs, 17 stage quest refs, 28 task snapshot-group refs, 28 task mission refs, 112 per-level Kite task reward refs, 9 milestone reward refs, 9 enemy target refs, 17 NPC target refs, 11 filter unlock item refs, and 17 sticker unlock item refs. Smoke checks passed for `snapshot_identify_group:snapshot_005`, `activity_snapshot_stage:activity_conditional_multistage_1_stage_1`, `snapshot_filter:filter_1`, and `kite_station_entrust_task:kitestation_002_1:1`. The normal source graph build is currently blocked by an unrelated dirty `scripts/build_data_index.py` import error, so this validation used a disposable `tmp/photo_chain_source_graph_runner.py` with a minimal decoder import stub. This models authored snapshot/group/filter/sticker/activity/Kite links; it does not prove runtime photo recognition, camera scoring, account unlocks, or live Kite Station task state.
