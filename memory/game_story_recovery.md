...
1. Resolve the mission/quest triggers for the 153 unassigned Story files that
   ...
   block metadata or the installed build changes. `sourceStoryGapQueue.v75`
   ...
   and reopen automatically if any gate changes. There are now 345 actual
   isolated-scene deferrals: 231 radios, 31 cutscene roots, 51 registered
   ...
   The e5m1 pass closes that frontier with the same exact gates. Four
   RadioTable definitions remain: `radio_e5m1_7` has three audio-complete
   lines, while `_10d8`, `_12`, and `_15` each have one; none has a typed
   carrier candidate. `dlg_e5m1_3` is a registered two-line/two-option
   DialogTree with exact primary/extra-config hashes and complete AudioDialog
   ...
   v52/v31 had exactly 286 declarations and 286 active deferrals. The
   main-story bucket was score 397 with 74 actionable core isolated rows, 105
   exact-native isolated closures, 92 exact-runtime-config closures, and 9
   strict-attachment diagnostics. e2m2 was next at score 25.
   The e2m2 pass adds a second reusable cross-owner class. Exact typed
   MissionRuntime quest client actions are imported across mission bundles
   only when relation, lifecycle phase, action slot/id/type, serialized
   Story-id source, and a unique nominal Story owner all agree.
   `radio_e2m2_1` closes through e2m1 `e2m1_q#3` succeed-slot
   `PlayRadio`; the generalized corpus pass also closes `radio_e2m3_3`
   through e2m2 q3 and `radio_e6m3_1` through e6m2 q25. These rows prove
   foreign mission/quest playback context, not ownership transfer or relative
   Story order. The remaining e2m2 keys are exact negative boundaries:
   ...
   seven-line/two-option `dlg_e2m2_4d5`. Their exact NpcProxyEx consumers are
   `tata_map01_i002[1]`, `fabian_map01_lv005[0]`, and
   `ailaizha_map01_lv005[0]`, all with empty mission ids. v53/v32 therefore
   has 289 declarations and 289 active deferrals. The main-story bucket is
   ...
   is next at score 20.
   The e6m2 pass closes its four-row frontier as exact current-build
   definition boundaries. `radio_e6m2_3` has one authored line and
   `radio_e6m2_7` preserves authored line order `_001`, `_004`, `_003`; all
   four radio-line audio ids are present. Registered `dlg_e6m2_1` preserves
   17 lines and five options, while `dlg_e6m2_2` preserves six lines and two
   options. Both require exact primary/extra DialogTree hashes, complete
   DialogTextTable and AudioDialog membership, and exact NpcProxyEx consumers:
   `zhuangfy_indie_dg005_e6m1Final[0]` has an empty `missionId`, and
   `mifu_indie_dg005_e6m1DianTiKou[2]` has no `missionId` field. These prove
   selectable NPC interaction definitions, not mission ownership, activation,
   or chronology. The carrier audit reports no typed candidate and all four
   generated route arrays remain empty. v56/v35 has 301 declarations and 301
   active deferrals. The main-story bucket is score 307 with 56 actionable
   core isolated rows, 105 exact-native isolated closures, 95
   exact-runtime-config closures, and 9 strict-attachment diagnostics. e3m4
   is next at score 20.
   The e3m4 pass closes four exact zero-route definition boundaries:
   audio-complete one-line `radio_e3m4_1` and `radio_e3m4_2`;
   DialogTextTable-only `dlg_e3m4_9`, whose one audio id is absent and which
   has no DialogId registration, DialogTree asset, or Timeline; and
   `cutscene_e3m4_1`, which consists of exactly 11 TextTable rows without a
   Timeline registry, root asset, object/PPtr carrier, or typed consumer.
   The four exported `f_/m_cs_video_e3m5_4.mp4` copies shown on the cutscene
   card are attached through an explicit WebUI presentation override. The
   authoritative `_moviePath` decode in LevelScript `2800010051` binds that
   FMV to `cutscene_e3m5_4` under e3m5, not to the e3m4 TextTable group; the
   override therefore does not establish activation, ownership, or order.
   v57/v36 has 305 declarations and 305 active deferrals. The main-story
   bucket is score 287 with 52 actionable core isolated rows, 105 exact-native
   ...
   The e9m4 pass closes that three-row frontier without turning name-shaped
   context into mission ownership. Registered `dlg_e9m4_14` preserves exactly
   seven authored lines (`_001` through `_006`, then `_009`), zero options,
   exact primary/extra DialogTree hashes, and complete AudioDialog membership.
   Its sole exact NpcProxyEx consumer is `lizhui_map02_e9m4[0]`; the entry has
   no `missionId` field, so the proxy-id suffix remains diagnostic naming
   rather than a serialized owner or activation time. `radio_e9m4_1` has one
   authored line and `radio_e9m4_4d5` has `_002`, `_003`, and `_004`; all four
   audio ids are present. All three Story manifest route arrays are empty and
   the current carrier audit reports no typed candidate. v59/v38 therefore
   has 310 declarations and 310 active deferrals. The main-story bucket is
   score 257 with 46 actionable core isolated rows, 106 exact-native isolated
   closures, 95 exact-runtime-config closures, and 9 strict-attachment
   diagnostics. e4m1 is now the first ranked main-story cluster at score 15.
   The e4m1 pass closes that cluster as three exact definition-only boundaries.
   `cutscene_e4m1_1` is exactly two TextTable rows (`_01` and `_02`) with no
   Timeline registry entry, root asset, GameObject/PPtr carrier,
   PlayableDirector host, structured action, Lua caller, or media override.
   Its manual Story-order placement remains display fallback only.
   `radio_e4m1_106` and `radio_e4m1_107` are one-line RadioTable definitions
   with complete AudioDialog membership. All three manifest entries have zero
   exact routes and the carrier audit reports no typed candidate. v60/v39 has
   313 declarations and 313 active deferrals. The main-story bucket is score
   242 with 43 actionable core isolated rows, 106 exact-native isolated
   closures, 95 exact-runtime-config closures, and 9 strict-attachment
   diagnostics. e1m4 is next at score 15.
   The e1m4 pass closes three residual radio definitions without conflating
   nearby exact LevelScript routes for `radio_e1m4_0d7`, `_1`, `_2`, `_3`,
   and `_4` with the distinct `_0d5`, `_1d5`, and `_2d5` keys.
   `radio_e1m4_0d5` and `_1d5` each have one authored line; `_2d5` has two.
   Their manifest routes are empty and the carrier audit reports no typed
   candidate. The `_0d5` base audio id is intentionally absent because
   AudioDialog contains exact `_f` and `_m` variants. v40 generalizes that
   distinction across all seven affected base ids in five deferred radio
   groups: `radio_e0m0_10`, `radio_e0m0_21`, `radio_e1m4_0d5`,
   `radio_e10m4_11`, and `radio_e10m4_38`. Each variant tuple must exactly
   match the current AudioDialog stems; these rows now publish
   `present_current_audio_dialog_variants` and an explicit `audioVariants`
   map rather than falsely reporting missing audio. v61/v40 has 316
   declarations and 316 active deferrals. The main-story bucket is score 227
   with 40 actionable core isolated rows, 106 exact-native isolated closures,
   95 exact-runtime-config closures, and 9 strict-attachment diagnostics.
   e2m3 is next at score 15.
   The e2m3 pass closes three exact zero-route RadioTable definitions:
   one-line `radio_e2m3_4`, two-line `radio_e2m3_6`, and one-line
   `radio_e2m3_15`. All four base audio ids are directly present in
   AudioDialog, the current carrier audit has no typed candidates, and no
   exact MissionRuntime, LevelScript, GameplayConfig, Lua, or Story-manifest
   route consumes them. v62/v41 has 319 declarations and 319 active
   deferrals. The main-story bucket is score 212 with 37 actionable core
   isolated rows, 106 exact-native isolated closures, 95 exact-runtime-config
   closures, and 9 strict-attachment diagnostics. e3m2 is next at score 15.
   The e3m2 pass preserves two different negative-definition states.
   Registered `dlg_e3m2_3` has exactly two lines, zero options, exact
   primary/extra DialogTree hashes, no Timeline, and an exact
   `angelu_map01_e3m201[0]` NpcProxyEx consumer whose entry has no
   `missionId`; both of its declared audio ids are genuinely absent, with no
   base or variant AudioDialog stems. `radio_e3m2_0d5` has five authored lines
   and `radio_e3m2_4d5` has one, all with direct AudioDialog membership.
   Every route array is empty and the carrier audit reports no typed
   candidate. v63/v42 has 322 declarations and 322 active deferrals. The
   main-story bucket is score 197 with 34 actionable core isolated rows, 106
   exact-native isolated closures, 95 exact-runtime-config closures, and 9
   strict-attachment diagnostics. e5m4 is next at score 15.
   The e5m4 pass closes the final score-15 main-story cluster as three
   definition-only radios: four-line `radio_e5m4_1`, three-line `_1d5`, and
   three-line `_2`. All ten declared audio ids are absent from AudioDialog,
   with no base or variant stems; v43 keeps true missing ids separate from the
   exact gender-variant tuples introduced in v40. These keys are also absent
   from the Story coverage denominator rather than present with empty routes.
   The Mission Pipeline overlay publisher now preserves their `radio` kind
   while retaining denominator-neutral, zero-route status. The carrier audit
   reports no typed candidates and the structured runtime/config scan has no
   exact consumers. v64/v43 has 325 declarations and 325 active deferrals.
   ...
   is next at score 10.
   The e10m2 pass closes two one-line, audio-complete definitions.
   `dlg_e10m2_8` has exact primary/extra DialogTree hashes, zero options,
   current DialogId registration, and no NpcProxyEx, MissionRuntime,
   LevelScript, Lua, object-index, or direct native playback consumer.
   `radio_e10m2_1` likewise has no recovered consumer. Both route arrays are
   empty and the carrier audit has no typed candidate. v68/v47 has 331
   declarations and 331 active deferrals. The main-story bucket is score 142
   ...
   is next at score 10.
   The e6m5 pass closes one audio definition and one multi-table text carrier.
   `radio_e6m5_4` has three authored RadioTable lines with complete
   AudioDialog membership and no recovered consumer. `text_e6m5_1` is exact
   ReadingPopUp row `rp_text_e6m5_1` plus a two-row RichContent payload.
   Two separately hash-gated archive tables, `PrtsAllItem` and `PrtsRecord`,
   also agree on `nar_collection_map02_69_1`: it targets this text under
   `collection_map02_69`, type `text`, order 1. That recovers a real PRTS
   archive carrier, but collection order is not mission chronology and the
   row serializes no mission owner or activator. Both Story keys remain
   carrier-negative and graph-neutral. v72/v51 has 339 declarations and 339
   active deferrals. The main-story bucket is score 102 with 15 actionable
   ...
   is next at score 10.
   The e5m5 pass closes two denominator-neutral RadioTable definitions.
   `radio_e5m5_1` has two authored lines and `radio_e5m5_2` has one; all
   three declared base audio ids are absent from AudioDialog. Neither key is
   present in the trigger-manifest denominator, and neither has a recovered
   MissionRuntime, LevelScript, GameplayConfig, Lua, object-index, or native
   playback consumer. The carrier audit also reports no candidate. v75/v54
   publishes both through the offline overlay as `radio`, zero-route,
   graph-neutral definitions. There are 345 active declarations; the
   main-story bucket is score 72 with 9 actionable core isolated rows, 108
   exact-native isolated closures, 95 exact-runtime-config closures, and 9
